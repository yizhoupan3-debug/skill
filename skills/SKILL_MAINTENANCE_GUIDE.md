...
- 是否需要刷新 Claude 可见入口 → 运行 `cargo run --manifest-path core/router-rs/Cargo.toml -- codex host-integration install-skills --repo-root \"$PWD\" install`（或使用已安装的 `router-rs` 等价命令），不要手动改 `~/.claude/commands`
