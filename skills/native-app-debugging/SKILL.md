...
- **Dual-Dimension Audit (Pre: Cross-Layer-Logic, Post: IPC-Fidelity/OS-Native Results)** → `$execution-audit-codex` [Overlay]
...
- **Superior Quality Audit**: For cross-boundary desktop apps, trigger `$execution-audit-codex` to verify against [Superior Quality Bar](../execution-audit-codex/references/superior-quality-bar.md).

## Trigger examples
- "强制进行原生应用深度审计 / 检查 IPC 通信与 OS 交互运行结果。"
- "Use $execution-audit-codex to audit this native app bridge for IPC-fidelity idealism."
