...
trigger_hints:
  ...
  - 宠物动画
do_not_use:
  - generic raster image generation that is not intended to become a Codex pet
  - standalone icon, logo, sticker, or illustration work without pet packaging
source: project
allowed_tools:
  - shell
  - python
...
This repo-local skill is a routing projection for the installed Hatch Pet
package. Keep routing metadata here so the framework can select `hatch-pet`,
but resolve executable helpers from the installed skill directory:

```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/hatch-pet"
```

Do not add deterministic helper scripts under this repo's `skills/hatch-pet/`
directory. The framework source tree intentionally stays free of project-local
Python control-plane scripts; the installed package owns those helpers.
