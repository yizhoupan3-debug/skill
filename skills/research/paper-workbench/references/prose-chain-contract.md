...
触发单真源：`has_paper_prose_edit_context`（hook 与 NL 共用）。受 `ROUTER_RS_OPERATOR_INJECT` 总闸约束；出站截断保留 `PAPER_*` 前缀行。实现：`core/research-harness/src/hooks/paper_prose.rs`、`core/framework-core/src/hook_common/mod.rs`。
