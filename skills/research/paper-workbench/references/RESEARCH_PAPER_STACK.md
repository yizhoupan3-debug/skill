...
**邻接技能**（按artifact 并入，不替代前门）：`statistical-analysis`（统计深度）、`experiment-reproducibility`（可复现）、`tikz-paper-figure`（作图代码）、`visual-review` / `pdf`（成稿视觉与版式）、`math-verify`（推导验证，窄）、`math-explore`（探索发现）。
