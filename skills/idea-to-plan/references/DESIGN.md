...
该 Skill 将作为所有复杂任务的“漏斗”入口，位于 `research-workbench` (发散) 和 `runtime checklist planning` (执行拆解收敛) 之上。

### 2.1 核心阶段 (The 5-Stage Blueprinting Loop)

1.  **Intake & Sourcing (意图摄入)**
    - 识别原始 Idea 的模糊度（Level 0-2）。
    - 联动 `research-workbench` 进行 360 度视角扩展。
...
3.5 **Multi-Direction Pilot (多向试点验证与快速判别)**
    - **Agentic Tree Search**：使用 `$runtime delegation gate` 开启并行分支。每个分支独立执行 `pilot_spec.json`。
    - **Recursive L0 Micro-Invocation**：Stage 3.5 **直接调用 `$runtime execution controller` (L0)** 运行“验证性微任务”。
...
4.  **Scientific & Integration Synthesis (科研与工程双重综合)**
    ...
    - **Spec Alignment**：只有在战略路径已经固定后，才调用 `$runtime checklist planning` 产出 execution checklist；不要让 checklist 反向替代战略 plan。
...
- **战略发散层 (L3)**：`$research-workbench`, `$literature-synthesis`.
- **可视化与交付层 (L4)**：`$scientific-figure-plotting`, `$image-generated`, `$source-slide-formats`.
- **核心内核 (L0)**：`$runtime execution controller` (递归调用入口)。
