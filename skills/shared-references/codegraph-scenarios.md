...
| **符号路径校验** | 对 ROADMAP 中标记的核心 symbol，调 `codegraph_node` 确认路径无漂移 | evidence 条目 |
| **breaking change** | 对已修改的公共 API 符号，调 `codegraph_callers[depth=3]` | 确认无意外 breaking change |
...
| 数据流追溯 | 对 diff 中可疑符号调 `codegraph_callers[depth=8]` | 完整上下游追溯 |
| PR 影响评估 | PR 删除公共函数/接口时，调 `codegraph_impact[depth=3]` | 评估下游破坏 |
| 符号定位 | diff 中符号不在当前文件时，调 `codegraph_search` | 定位定义位置 |
