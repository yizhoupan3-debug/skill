...
- The user wants one front door for a research-project task rather than reproducibility only -> use `$research-execution` and keep this skill only for reproducibility constraints
...
- `$research-execution`, project owners, and experiment owners may use this skill as the rigor / reproducibility lane
...
- If the user only asks "下一步做什么", do not turn that into a reproducibility-only task; return to `$research-execution` with only the reproducibility blocker noted
