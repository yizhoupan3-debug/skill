...
- The user wants one front door for a research-project task rather than reproducibility only -> use `$research` (execution lane) and keep this skill only for reproducibility constraints
...
- Manuscript workflow map: [`../research/paper-workbench/references/RESEARCH_PAPER_STACK.md`](../research/paper-workbench/references/RESEARCH_PAPER_STACK.md)
- **科研纪录最低清单**（预注册、主/探索分析、方案偏离、数据 DOI、软件 pin）：[`references/research-record-minimum.md`](references/research-record-minimum.md)；与仓库连续性（`artifacts/current/`、`SESSION_SUMMARY` / `NEXT_ACTIONS` / `EVIDENCE_INDEX`）对齐见 [`../research/paper-workbench/references/RESEARCH_PAPER_STACK.md`](../research/paper-workbench/references/RESEARCH_PAPER_STACK.md) §科研纪录与仓库连续性
- `$research` (execution lane), project owners, and experiment owners may use this skill as the rigor / reproducibility lane
...
- If the user only asks "下一步做什么", do not turn that into a reproducibility-only task; return to `$research` (execution lane) with only the reproducibility blocker noted
