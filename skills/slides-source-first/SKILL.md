...
description: |
  ...
  Use this for workflow choice after the `$slides` gate; keep existing-deck native edits on `$slides`
  and source-first `.pptx` authoring in `$ppt-pptx`.
routing_layer: L2
routing_owner: owner
routing_gate: none
session_start: n/a
trigger_hints:
  ...
  - cross-format slide maintenance
  - source first
  - one source of truth for slides
  - cross-format slide workflow
metadata:
  version: "1.0.0"
  platforms: [codex]
  tags:
...
- Existing `.pptx` inspection or in-place patching -> use `$slides`
