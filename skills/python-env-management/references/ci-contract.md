...
**Status (2026-05-20):** Root `pyproject.toml` + `uv.lock`; `.github/workflows/skill-ci.yml` uses `astral-sh/setup-uv@v5`, `uv sync --frozen`, and `uv run python` for hook tests. Local runs: `ROUTER_RS=./scripts/router-rs/target/debug/router-rs uv run python .cursor/hook-tests/test_install_codex_cli_hooks.py`.
