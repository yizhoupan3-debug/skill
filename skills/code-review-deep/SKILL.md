...
**ANTIGRAVITY_REVIEW_GATE clearance (Antigravity)**: 依靠 `goal_state_manage` 的 MCP Advisory 硬拦截实现。**没有** CLI 级的 `subagentStart/Stop` hook 机制。主代理通过原生 `invoke_subagent` 派生子代理并收到其 `send_message` 的结果后，**必须手动将 Findings 写入** `artifacts/current/<task_id>/review-lanes/*.md`（或在 `closeout_gate` 显式传入 `review_evidence_attested=true`）。系统仅扫描 `review-lanes` 目录作为唯一硬凭证（物理 `.md` 文件是 **hard requirement**），缺失时强制触发 `[Antigravity Hard Block]` 驳回 `complete` 核销操作。`lifecycle_profile: my-light` 关闭硬拦。
