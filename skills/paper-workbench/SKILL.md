...
- `@lane:reviewer` prose quality checks → Prose Quality Check（本文档内）
- `@lane:reviewer` structure/logic checks → Structure Check（本文档内）
- Literature/citation integrity checks → [`../../citation-management/SKILL.md`](../../citation-management/SKILL.md)
- Statistical methodology checks → [`../../statistical-analysis/SKILL.md` (Verify Audit Mode)](../../statistical-analysis/SKILL.md)
...
## Structure Check（LaTeX 编译/交叉引用审计）

论文文档结构的完整性检查。

| 检查项 | 说明 | 命令 |
|--------|------|------|
| LaTeX 编译 | 检查 TeX 文件能否无错误编译 | `research_verification_structure(check="latex", path="...")` |
| 图表引用 | 检查图/表引用是否全部存在对应对象 | `research_verification_structure(check="figures", path="...")` |

## Prose Quality Check（术语/套话/声明漂移审计）

论文文稿的语言质量门禁检查。**不是改稿，不是润色**——只做审计（PASS/FAIL/WARN）。

### 检查维度

| 维度 | 检查内容 | 工具 |
|------|---------|------|
| 术语一致性 | 同一概念在全文是否用词一致 | `research_verification_prose(check="terminology")` |
| 学术套话检测 | 是否过度使用 AI 高频句式（slop） | `research_verification_prose(check="slop")` |
| Hedging 适度性 | 限定词使用是否过强或过弱 | `research_verification_prose(check="hedging")` |

### 约束

- 仅执行验证，不做修改
- 套话/slop 检测需指定语言（en/zh）
- 术语表可选（提供 glossary 时做即席术语归一）
...
> 如需更强降重（含声音校准、LLM 增强语义改写），转 `$humanizer` 做二次处理。
- **保持学术语气**：不降低学术规范性
