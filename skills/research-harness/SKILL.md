---
description: Research workspace CLI — initialize directions, manage claims/hypotheses, record runs, reflect, escalate barriers, smoke test experiments. Backed by core/research-harness crate and autoresearch CLI binary.
metadata:
  platforms:
  - supported
  tags:
...
short_description: 科研工作区 — 方向初始化/claim/假设/实验记录/smoke test/barrier 升级
trigger_hints:
- 研究工作区
- 科研工作区
- research workspace
...
# Research Workspace — First-Class Skill
...
- The user says "突破不了"/"瓶颈"/"被卡住"/"stuck" in an auto-experimentation context.
- The user wants **structured research logging** (text layer + SQLite FTS5).
- The user needs **smoke tests** for quick experiment probes.
- The user asks about **experiment feasibility or barrier analysis**.
...
- **统一科研前门**: `$research` (slug `research`, path `skills/research/SKILL.md`)
- **Discovery lane**: `$research` (discovery lane) — literature, theory
- **Execution lane**: `$research` (execution lane) — experiment design, math
- **Paper manuscript**: `$research` (paper-workbench lane) — manuscript review/revise
