...
- **有范围后缀**（例如 **`/gitx core/router-rs`** 或 **`/gitx plan core/router-rs`**、或一句明确议题）：两种前缀等价；仍以全仓诊断命令看清全局，但 **深度 review、修复、拆分/整理提交面、验证与最终纳入提交的 path** 只针对用户给出的范围；**不要把明显无关路径的改动顺手塞进同一提交**，除非为消除该范围内的构建/测试失败所必需。
