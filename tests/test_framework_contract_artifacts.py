...
def test_emit_framework_contract_artifacts_writes_parity_snapshot_baseline_and_rust_outputs(
    ...
    assert cli_parity["cli_hosts"]["claude_code_adapter"]["mcp_config_paths"] == [
        "~/.claude.json",
        ".mcp.json",
    ]
    ...
    assert alias_inventory["summary"]["translation_shim_required"] is False
    ...
    assert alias_retirement["primary_regression_artifact"] == "cli_family_parity_snapshot"
    assert alias_retirement["codex_dual_entry_compatibility_artifact"] == "codex_dual_entry_parity_snapshot"
    assert alias_retirement["retirement_gates"]["runtime_primary_identity_consumers_cleared"] is True
    assert alias_retirement["emitter_contract"]["python_emits_alias_artifact"] is False
    assert alias_retirement["emitter_contract"]["rust_emits_alias_artifact"] is False
    ...
    assert supervisor_state["status_contract"] == "supervisor_state_contract_v1"
    ...
    assert (
        fallback_retirement["current_response_metadata_truth"][
            "compatibility_fallback_reason_present_only_on_fallback"
        ]
        is True
    )
    ...
    assert response_serialization["current_response_shape_truth"]["compatibility_fallback"][
    ...
    assert rust_supervisor["status_contract"] == "supervisor_state_contract_v1"
    ...
    assert rust_response_serialization["current_response_shape_truth"]["compatibility_fallback"][
