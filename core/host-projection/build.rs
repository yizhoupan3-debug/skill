...
fn main() {
    ...
    // ── Projection Ops Struct Definitions ──
    // Generate per-host struct definitions + impl HostProjectionOps,
    // driven by RUNTIME_REGISTRY.json host_targets.supported.
    // Included via `include!` in projection_host_ops.rs.
    let mut struct_out = String::new();
    struct_out.push_str("// @generated by build.rs from configs/framework/RUNTIME_REGISTRY.json\n");
    struct_out.push_str("// ProjectionOps struct definitions -- do not edit manually.\n\n");
    for id in &supported {
