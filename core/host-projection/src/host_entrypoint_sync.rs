...
fn sync_host_entrypoint_file(
    ...
    let destination = router_rs::path_guard::join_repo_relative_under_root(target_root, relative)?;
    ...
    if changed && apply {
        ...
        router_rs::atomic_write::write_atomic_text(&destination, text)?;
    }
...
mod tests {
    ...
    fn host_entrypoint_sync_engine_accepts_fake_provider_in_dry_run() {
        ...
        fs::write(
            &registry_json,
            r#"{"schema_version":"framework-runtime-registry-v1","host_targets":{"supported":["codex","codex","cursor","claude-code"],"metadata":{"codex":{"install_tool":"codex","projection_status":"implemented","installable":true,"host_entrypoints":"AGENTS_CODEX.md"},"codex":{"install_tool":"codex","projection_status":"implemented","installable":false,"host_entrypoints":"AGENTS_CODEX.md"},"cursor":{"install_tool":"cursor","projection_status":"implemented","installable":true,"host_entrypoints":["AGENTS_CURSOR.md",".cursor/rules/*.mdc"]},"claude-code":{"install_tool":"claude","projection_status":"implemented","installable":true,"host_entrypoints":["AGENTS_CLAUDE.md",".claude/rules/framework.md",".claude/settings.json"]}}}}"#,
        )
        .unwrap();
        assert!(
