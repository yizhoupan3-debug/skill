...
use router_rs::router_env_flags::router_rs_env_enabled_default_true;
...
impl From<CodexHookError> for framework_core::error::FrameworkError {
    fn from(err: CodexHookError) -> Self {
        Self::state(err.to_string())
    }
}

pub fn codex_merge_legacy_subagent_gate_evidence(state: &mut CodexLifecycleContextState) {
...
pub fn codex_state_dir(repo_root: &Path) -> PathBuf {
    repo_root
...
pub fn codex_stable_session_raw(event: &Value) -> Option<String> {
...
pub fn codex_require_stable_session_key_enabled() -> bool {
...
pub fn codex_unstable_session_key_raw(repo_root: &Path, event: &Value) -> String {
...
pub fn codex_session_key(repo_root: &Path, event: &Value) -> String {
...
pub fn codex_state_path(repo_root: &Path, event: &Value) -> PathBuf {
    codex_state_dir(repo_root)
        .join(format!("review-subagent-{}.json", codex_session_key(repo_root, event)))
}

pub fn parse_lock_metadata(text: &str) -> (Option<u32>, Option<u64>) {
    let mut pid = None;
    let mut ts = None;
...
pub fn lock_is_stale(path: &Path) -> bool {
    ...
    ts.map_or(true, |t| now_ms.saturating_sub(t) > 30_000)
}

#[cfg(unix)]
pub fn acquire_codex_state_lock(state_path: &Path) -> Result<CodexStateLock, CodexHookError> {
...
pub fn codex_load_state_from_path(path: &Path) -> Result<Option<CodexLifecycleContextState>, CodexHookError> {
    let text = match fs::read_to_string(path) {
        Ok(value) => value,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(_) => return Err(CodexHookError::StateReadIo(io::Error::other("state_read_failed"))),
    };
...
pub fn codex_save_state_to_path(state_path: &Path, state: &CodexLifecycleContextState) -> bool {
...
pub fn prune_stale_hook_state_files(dir: &Path) {
...
pub fn with_codex_state_lock<T, F>(repo_root: &Path, event: &Value, f: F) -> Result<T, CodexHookError>
where
    F: FnOnce(
