...
fn truncate_codex_additional_context_bytes(combined: &str, max_bytes: usize) -> String {
    hooks::truncate_hook_outbound_lines_preserving(
        combined,
        max_bytes,
        "...",
    )
...
fn handle_codex_userpromptsubmit(repo_root: &Path, event: &Value) -> Option<Value> {
    ...
    hooks::maybe_append_paper_adversarial_context(
        repo_root,
        &prompt,
        &mut contexts,
        paper_host,
    );
    hooks::maybe_append_paper_prose_context(
        repo_root,
        &prompt,
        &mut contexts,
        paper_host,
...
///
/// Two intentional differences vs [`core_state::utils::atomic_write::write_atomic_text`]:
///
...
fn attach_codex_hook_observation(mut value: Option<Value>) -> Option<Value> {
    if let Some(ref mut v) = value {
        hooks::attach_router_rs_observation(
            v,
            hooks::HookObservationHost::Codex,
        );
    }
    value
}
...
"#,
    ...
    mod lifecycle_context_tests {
        ...
        fn env_lock() -> core_policy::test_env_sync::ProcessEnvLockGuard {
