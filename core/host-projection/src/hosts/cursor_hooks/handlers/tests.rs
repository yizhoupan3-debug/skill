use super::*;
use router_rs::hosts::cursor_hooks::resolve_cursor_hook_repo_root;
...
impl ReviewGateDisableTestGuard {
    fn new() -> Self {
set_test_review_gate_disable_override(Some(true));
        Self
    }
}
...
impl Drop for ReviewGateDisableTestGuard {
    fn drop(&mut self) {
set_test_review_gate_disable_override(None);
    }
}
...
impl OperatorInjectEnabledGuard {
    fn new() -> Self {
        let lock = router_rs::test_env_sync::process_env_lock();
...
impl ForceHookStateLockFailureGuard {
    fn new() -> Self {
set_force_cursor_hook_state_lock_failure(true);
        Self
    }
}
...
impl Drop for ForceHookStateLockFailureGuard {
    fn drop(&mut self) {
set_force_cursor_hook_state_lock_failure(false);
    }
}
...
fn assert_followup_signals_review_gate_incomplete(blob: &str) {
    ...
    assert!(
        blob.contains(REVIEW_GATE_FOLLOWUP_NEED_SEGMENT),
        "expected need segment `{}` in {blob:?}",
REVIEW_GATE_FOLLOWUP_NEED_SEGMENT
    );
    assert!(
        blob.contains(REVIEW_GATE_FOLLOWUP_HINT_SEGMENT),
...
fn review_gate_stdout_scrub_drops_spoof_rg_followup_missing_parts() {
    ...
    router_rs::goal_state::scrub_followup_fields_in_hook_output(&mut out);
...
fn session_key_matches_when_parent_session_only_in_tool_input() {
    ...
    assert_eq!(session_key(&full), session_key(&tool_only));
}
...
fn session_key_prefers_root_session_over_nested_child_conversation() {
    ...
    assert_eq!(session_key(&mixed), session_key(&stable));
}
...
fn cursor_session_key_child_conversation_with_parent_session_id_in_tool_input() {
    ...
    assert_eq!(
session_key(&subagent_only),
session_key(&parent_top),
...
fn session_key_ignores_lonely_agent_id_for_cwd_fallback_match() {
    ...
    assert_eq!(
session_key(&only_agent),
session_key(&cwd_only)
    );
}
...
fn write_active_task(repo: &Path, task_id: &str) {
    ...
    fs::create_dir_all(repo.join(ARTIFACTS_CURRENT_DIR).join(task_id)).expect("mkdir task dir");
}
...
fn write_goal_state_completed(repo: &Path, task_id: &str) {
    fs::write(
        repo.join(ARTIFACTS_CURRENT_DIR)
            .join(task_id)
...
fn review_goal_mixing_nudge_predicate_requires_non_my_light() {
    ...
    router_rs::hook_common::set_test_my_light_override(None);
    assert!(
        router_rs::hook_common::my_light_profile_active(None, "/implementx"),
        "implementx in prompt always activates my-light profile"
    );
    assert!(
        !router_rs::hook_common::my_light_profile_active(None, "全面review这个仓库"),
...
fn before_submit_implementx_returns_unreadable_when_hook_state_corrupt() {
    ...
    let msg = out
        ...
        .unwrap_or("");
    assert!(
        msg.contains(CURSOR_HOOK_STATE_UNREADABLE),
...
fn stop_completion_claim_requires_closeout_record_when_strict_enabled() {
    ...
    fs::write(
            repo.join(ARTIFACTS_CURRENT_DIR).join(tid).join("EVIDENCE_INDEX.json"),
...
fn stop_closeout_uses_hydration_task_when_active_completed_and_focus_running() {
    ...
    for id in ["done-active", "drive-focus"] {
        fs::create_dir_all(repo.join(ARTIFACTS_CURRENT_DIR).join(id)).expect("mkdir");
    }
    fs::write(
...
fn my_skips_pre_goal_nag_when_goal_state_on_disk() {
    ...
    .expect("active");
    router_rs::goal_state::framework_goal_drive(json!({
...
fn stop_lock_failure_reports_degraded_followup() {
    ...
    let blob = hook_user_visible_blob(&out);
    assert!(
        blob.contains("hook_state_lock_degraded"),
...
fn stop_with_active_goal_does_not_inject_goal_continue() {
    ...
    fs::create_dir_all(repo.join(ARTIFACTS_CURRENT_DIR).join(tid)).expect("mkdir goal");
    fs::write(
        repo.join("artifacts/current/active_task.json"),
...
fn before_submit_does_not_merge_goal_or_rfv_continuity() {
    let repo = fresh_repo();
    let tid = "merge-both";
    fs::create_dir_all(repo.join(ARTIFACTS_CURRENT_DIR).join(tid)).expect("mkdir");
    fs::write(
        repo.join("artifacts/current/active_task.json"),
        format!(r#"{{"task_id":"{tid}"}}"#),
    )
...
fn stop_goal_and_rfv_do_not_merge_schema_hint_into_continue() {
    ...
    assert!(
        !blob.contains(router_rs::rfv_loop::RFV_EXTERNAL_RESEARCH_SCHEMA_REL_PATH),
...
fn cursor_hook_output_policy_truncates_additional_context_under_env_budget() {
    ...
    assert!(
        s.starts_with("AUTOPILOT_HEAD")
            ...
            && s.ends_with(CURSOR_HOOK_OUTBOUND_TRUNC_SUFFIX),
...
fn cursor_hook_output_policy_truncates_followup_after_absurd_length() {
    ...
    assert!(s.ends_with(CURSOR_HOOK_OUTBOUND_TRUNC_SUFFIX));
...
fn cursor_hook_outbound_trunc_respects_byte_cap_and_marker() {
    ...
    let got = truncate_cursor_hook_outbound_context(&body, max_out);
    assert!(got.len() <= max_out, "len {} max {}", got.len(), max_out);
    assert!(got.ends_with(CURSOR_HOOK_OUTBOUND_TRUNC_SUFFIX));
}
...
REVIEW_GATE_FOLLOWUP_HINT_SEGMENT
    ...
    let max_out = router_rs::router_env_flags::router_rs_cursor_hook_outbound_context_max_bytes();
    let got = truncate_cursor_hook_outbound_context_preserving_gate(&body, max_out);
    assert!(got.len() <= max_out);
    assert!(got.contains("continuity_suppressed=review_soft_nag"));
    assert!(got.contains(REVIEW_GATE_FOLLOWUP_NEED_SEGMENT));
...
fn post_tool_skips_cargo_check_when_env_off() {
    ...
    assert!(
        !router_rs::router_env_flags::router_rs_cursor_cargo_check_sync_enabled(),
...
impl MyLightOverrideGuard {
    fn force_non_my_light() -> Self {
        router_rs::hook_common::set_test_my_light_override(Some(false));
        Self
    }

    fn force_my_light() -> Self {
...
fn deep_reviewer_lane_counts_for_review_gate() {
    let _gate = ReviewGateActiveGuard::new();
    assert!(router_rs::hook_common::is_deep_review_gate_lane_normalized("deep-reviewer"));
    let repo = fresh_repo();
...
fn cursor_hook_rejects_non_object_stdin() {
    ...
    let err = router_rs::hosts::cursor_hooks::stdin::read_stdin_json_from_reader(&mut reader)
...
fn signal_pid_or_pgrp_never_targets_our_process_group() {
    ...
    let our_pgid = current_pgid().expect("current pgid");
    // 发送 0 信号不产生副作用，但会触发 syscall 分支。
signal_pid_or_pgrp(our_pid, Some(our_pgid), 0);
}
...
fn session_start_operator_inject_off_skips_additional_context() {
    ...
    env::set_var("ROUTER_RS_OPERATOR_INJECT", "0");
    let repo = fresh_repo();
    fs::create_dir_all(repo.join(ARTIFACTS_CURRENT_DIR)).expect("mkdir ac");
    fs::write(
...
fn session_start_resets_session_call_tracker() {
    let _inject_on = OperatorInjectEnabledGuard::new();
    let repo = fresh_repo();
    fs::create_dir_all(repo.join(ARTIFACTS_CURRENT_DIR)).expect("mkdir");
    router_rs::session_call_tracker::init_tracker(&repo).expect("seed");
    for _ in 0..50 {
        router_rs::session_call_tracker::record_tool_call(&repo, "Read").expect("record");
    }
    let before = router_rs::session_call_tracker::read_tracker_state(&repo).expect("read");
    ...
    let after = router_rs::session_call_tracker::read_tracker_state(&repo).expect("read after");
...
fn session_start_repo_only_no_continuity_hints() {
    ...
    let cur = repo.join(ARTIFACTS_CURRENT_DIR);
    ...
    assert!(
        !ctx.contains(router_rs::task_state::CONTINUITY_ACTIVE_FOCUS_GOAL_MISMATCH_HINT_ZH),
...
fn cursor_launcher_fail_closed_before_submit_when_router_rs_missing() {
    ...
    // Copy _resolve_router_rs.sh so the hook script can source it
    let resolve_rs = framework.join("configs/framework/_resolve_router_rs.sh");
    let fw_dir = empty_ws.join("configs/framework");
    fs::create_dir_all(&fw_dir).expect("fw dir");
    fs::copy(&resolve_rs, fw_dir.join("_resolve_router_rs.sh")).expect("copy _resolve_router_rs.sh");
    // Use empty HOME so ~/.local/bin/router-rs is not discovered
    let empty_home = empty_ws.join("fake-home");
    fs::create_dir_all(&empty_home).expect("fake home");
...
fn terminal_observation_cache_avoids_repeat_dir_scan() {
    use router_rs::hosts::cursor_hooks::terminal_observation_cache::{
    ...
    for i in 0..num_threads {
        ...
        threads.push(std::thread::spawn(move || {
            ...
            for j in 0..10 {
                ...
                let _ = router_rs::atomic_write::write_atomic_json(&path_clone, &val);
                // 模拟一点延迟以增加重合度
                std::thread::sleep(std::time::Duration::from_millis(2));
            }
        }));
...
fn review_gate_mode_strict_regression_unset_env() {
    ...
    assert_eq!(
        router_rs::review_gate_engine::cursor_review_gate_mode(),
        router_rs::review_gate_engine::CursorReviewGateMode::Strict
    );
}
