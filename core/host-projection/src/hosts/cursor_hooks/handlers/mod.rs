...
use router_rs::review_gate_engine::{fork_context_from_values, review_gate_armed};
use router_rs::review_output_lint::{lint_review_output, LintSeverity};
use router_rs::runtime_envelope_ids::MAX_CONCURRENT_SUBAGENTS_LIMIT;
...
use router_rs::hosts::cursor_hooks::subtraction;
...
fn merge_session_close_style_nudge_when_soft_terminal(output: &mut Value) {
    ...
    if !router_rs::router_env_flags::router_rs_operator_inject_globally_enabled() {
        return;
    }
    ...
    router_rs::goal_state::merge_hook_nudge_paragraph(
        output,
        msg,
...
fn release_lock_then_finalize_stop(
    repo_root: &Path,
    output: &mut Value,
    frame: &router_rs::task_state::HostContinuityFrame,
...
fn finalize_stop_hook_outputs(
    _repo_root: &Path,
    output: &mut Value,
    _frame: &router_rs::task_state::HostContinuityFrame,
) {
...
fn completion_claimed_in_text(text: &str) -> bool {
    ...
    router_rs::hook_common::contains_completion_claim_token(&sanitized)
}
...
fn closeout_followup_for_completion_claim(
    ...
    if !router_rs::framework_runtime::closeout_programmatic_enforcement_enabled() {
        return Ok(None);
    }
    let record_path = router_rs::framework_runtime::closeout_record_path_for_task(repo_root, task_id).map_err(|e| e.to_string())?;
...
- 评估命令：router-rs closeout evaluate --repo-root \"{}\" --task-id \"{}\" --record-path \"{}\"",
    ...
    let eval = router_rs::framework_runtime::evaluate_closeout_record_file_for_task(
        repo_root,
        task_id,
        &record_path,
...
fn stop_hard_closeout_followup_for_assistant_response(
    ...
    let tid = router_rs::task_state::resolve_continuity_frame(repo_root)
        .hydration_goal
...
fn goal_chat_verify_zh_signal(text: &str) -> bool {
    router_rs::hook_common::GOAL_CHAT_VERIFY_ZH_PHRASES
        .iter()
        .any(|p| text.contains(p))
}

/// Task/subagent 调用里明示 `fork_context: true` 时视为与主会话共享上下文，不满足 goal 要求的「独立上下文」预检。
...
fn cursor_fork_context_from_tool(
    ...
    if !router_rs::router_env_flags::router_rs_cursor_review_fork_context_missing_infer_false_enabled()
    {
        return None;
    }
    ...
    if router_rs::hook_common::is_deep_review_gate_lane_normalized(lane) {
        Some(false)
    } else {
        None
    }
...
fn review_subagent_kind_ok(sub_type: &str, agent_type: &str) -> bool {
    ...
    (!sub_type.is_empty() && router_rs::hook_common::is_deep_review_gate_lane_normalized(sub_type))
        || (!agent_type.is_empty()
            && router_rs::hook_common::is_deep_review_gate_lane_normalized(agent_type))
}
...
fn hook_event_signal_text_with_scrape_mode(
    ...
    let response = router_rs::hook_common::hook_assistant_tail_window(
        response,
...
fn grab_tool_input_from_object(obj: &serde_json::Map<String, Value>) -> Option<Value> {
    router_rs::hook_common::tool_input_value_from_map(obj)
}
                        ...
                        if let Some(s) = child.as_str() {
                            ...
                            if !t.is_empty() {
                                *ties += 1;
                                ...
                                if pick
                                    .as_ref()
                                    .map_or(true, |(bp, bo, _)| pi < *bp || (pi == *bp && ord < *bo))
                                {
...
pub fn session_key(event: &Value) -> String {
...
fn framework_prompt_arms_delegation(text: &str) -> bool {
    router_rs::hook_common::is_framework_non_goal_entrypoint_prompt(text)
}
...
fn save_session_terminal_ledger(repo_root: &Path, event: &Value, ledger: &SessionTerminalLedger) {
    ...
    if let Ok(text) = serde_json::to_string_pretty(&ledger) {
        let _ = router_rs::atomic_write::write_atomic_text(&path, &text);
    }
}
...
fn merge_additional_context(output: &mut Value, extra: &str) {
    let extra = router_rs::goal_state::scrub_spoof_host_followup_lines(extra);
    match output.get_mut("additional_context") {
        Some(Value::String(s)) => {
            s.push_str("\n\n");
            s.push_str(&extra);
            *s = router_rs::goal_state::scrub_spoof_host_followup_lines(s);
        }
        _ => {
...
fn acquire_state_lock(repo_root: &Path, event: &Value) -> Option<LockGuard> {
    ...
    #[cfg(unix)]
    {
        ...
        let retries = router_rs::router_env_flags::router_rs_cursor_hook_state_lock_retries();
        for _ in 0..retries {
            ...
            match file.try_lock_exclusive() {
                Ok(()) => {
                    ...
                    router_rs::hook_timing::add_lock_wait_ms(wait_start.elapsed().as_millis() as u64);
...
pub fn load_state(repo_root: &Path, event: &Value) -> Result<Option<ReviewGateState>, String> {
...
fn save_state(repo_root: &Path, event: &Value, state: &mut ReviewGateState) -> bool {
    ...
    if router_rs::router_env_flags::router_rs_cursor_hook_state_file_sync_enabled()
    ...
    #[cfg(unix)]
    if router_rs::router_env_flags::router_rs_cursor_hook_state_dir_sync_enabled() {
...
fn maybe_bump_review_phase_for_main_thread_compact_findings(
    ...
    if !router_rs::review_output_lint::assistant_has_substantive_compact_review_finding_line(
        assistant_tail,
    ) {
        return false;
    }
...
fn review_stop_followup_line(state: &ReviewGateState) -> String {
    let cap_need = if state.review_pending_cap_refused {
        format!(
            ...
            router_rs::router_env_flags::router_rs_cursor_review_pending_cycle_max()
        )
    } else {
        String::new()
    };
...
pub const CURSOR_HOOK_STATE_UNREADABLE: &str =
    "router-rs CURSOR_HOOK_STATE_UNREADABLE need=repair_hook_state_json fix=rm -f .cursor/hook-state/*.json";
...
/// - 正整数：自定义上限（运维 opt-in）。
fn cursor_goal_pre_max_nudges_cap() -> Option<u32> {
...
fn maybe_goal_pre_nag_cap_release(state: &mut ReviewGateState) -> Option<&'static str> {
    if !router_rs::router_env_flags::router_rs_cursor_goal_pre_enabled() {
        return None;
    }
    if !tracks_goal_or_drive_entry(state)
    ...
    let cap = cursor_goal_pre_max_nudges_cap()?;
...
fn cursor_review_gate_disabled_by_env() -> bool {
    ...
    router_rs::router_env_flags::router_rs_env_enabled_default_false(
...
pub fn apply_cursor_hook_silent_policy(output: &mut Value) {
    if !router_rs::router_env_flags::router_rs_cursor_hook_silent_enabled() {
        return;
    }
...
pub fn apply_cursor_hook_output_policy(output: &mut Value) {
    router_rs::router_rs_observation::attach_router_rs_observation(
        output,
        router_rs::router_rs_observation::HookObservationHost::Cursor,
    );
    let max_out = router_rs::router_env_flags::router_rs_cursor_hook_outbound_context_max_bytes();
    ...
    let absurd_followup_threshold =
        router_rs::router_env_flags::router_rs_cursor_hook_outbound_context_max_bytes()
            .saturating_mul(4)
            .max(32 * 1024);
    if let Some(Value::String(s)) = output.get_mut("followup_message") {
...
pub const CURSOR_HOOK_OUTBOUND_TRUNC_SUFFIX: &str = "...[~trunc]";
...
fn cursor_hook_outbound_line_is_protected(line: &str) -> bool {
    router_rs::hook_outbound_protect::hook_outbound_line_is_framework_protected(line)
}
...
pub fn truncate_cursor_hook_outbound_context_preserving_gate(
    combined: &str,
    max_bytes: usize,
) -> String {
...
fn strip_cursor_hook_user_visible_nags(output: &mut Value) {
    if let Some(obj) = output.as_object_mut() {
        obj.remove("followup_message");
        obj.remove("additional_context");
        router_rs::router_rs_observation::strip_router_rs_observation(output);
    }
}
...
fn hydrate_goal_gate_from_disk(
    ...
    if !router_rs::router_env_flags::router_rs_cursor_pre_goal_strict_disk_enabled()
    ...
    if state.goal_required || arm_if_goal_file || state.goal_drive_entry_active {
        let readiness = router_rs::ship_readiness::evaluate_goal_readiness_from_disk(
            repo_root,
            goal,
            task_id.as_str(),
        );
...
fn goal_stop_followup_line(state: &ReviewGateState) -> String {
    router_rs::ship_readiness::goal_stop_followup_line(
...
fn state_lock_degraded_followup() -> &'static str {
    "router-rs REVIEW_GATE incomplete hook_state_lock_degraded fix=rm -f .cursor/hook-state/*.json && retry Stop | 或用户消息单独一行 rg_clear"
}
...
fn try_settle_review_subagent_cycle(
    ...
    let lite = router_rs::review_gate_engine::cursor_review_gate_mode()
        == router_rs::review_gate_engine::CursorReviewGateMode::Lite
        && router_rs::review_gate_engine::cycle_key_eligible_for_lite(k);
    let pending = if lite {
...
fn push_review_lite_pending_cycle_key(
    ...
    let max = router_rs::router_env_flags::router_rs_cursor_review_pending_cycle_max();
...
fn push_review_pending_cycle_key(
    ...
    if router_rs::review_gate_engine::cursor_review_gate_mode()
        == router_rs::review_gate_engine::CursorReviewGateMode::Lite
    {
        if router_rs::review_gate_engine::cycle_key_eligible_for_lite(&k) {
            if lite_stable_id {
...
fn handle_before_submit(repo_root: &Path, event: &Value) -> Value {
    let frame = router_rs::task_state::resolve_continuity_frame(repo_root);
    ...
    let my_light = router_rs::hook_common::my_light_profile_active(Some(repo_root), &text);
    ...
    // 用户在本轮提交里写出 reject_reason token 时须即时生效；否则仅能在助手回复或 Stop 里识别，导致 goal pre-goal 与 AG_FOLLOWUP 循环。
    ...
    let pre_goal_auto_release_note = maybe_goal_pre_nag_cap_release(&mut state);
    ...
    let needs_goal_pre =
        router_rs::router_env_flags::router_rs_cursor_goal_pre_enabled()
    ...
    if review_arms_for_gate
        ...
        && router_rs::hook_common::should_inject_spawn_first_review_nudge(Some(repo_root), &text)
    {
        let nudge =
            router_rs::runtime_registry::review_spawn_first_nudge_line(Some(repo_root), "cursor");
        spawn_first_line = Some(nudge.clone());
        merge_additional_context(&mut output, &nudge);
    }
    let skip_model_nudge = spawn_first_line.is_some()
        && router_rs::runtime_registry::spawn_first_includes_model_inherit_for_host(
            Some(repo_root),
            "cursor",
        );
    if !my_light
    ...
    if !skip_model_nudge
        && router_rs::hook_common::should_inject_subagent_model_inherit_nudge(
            &text,
            user_gate_override,
            goal_drive_entrypoint,
            delegation,
    ...
    {
        let model_nudge = router_rs::runtime_registry::review_subagent_model_inherit_nudge_line(
            Some(repo_root),
            "cursor",
        );
    ...
    if review
        && router_rs::review::heterogeneous::should_plan_heterogeneous_adversarial_lane(&text, true)
    {
        if let Some(hint) = router_rs::review::heterogeneous::heterogeneous_review_hint_for_lane() {
            merge_additional_context(&mut output, &hint);
        }
    }
    if needs_goal_pre {
    ...
    if router_rs::hook_common::is_my_pre_execution_entry_prompt(&text) {
        merge_additional_context(&mut output, router_rs::hook_common::MY_PRE_EXECUTION_HOOK_NUDGE);
    }
    if goal_drive_entrypoint {
        merge_additional_context(
            &mut output,
            router_rs::hook_common::my_goal_drive_hook_nudge_for_prompt(&text),
        );
    }
    router_rs::paper_adversarial_hook::maybe_merge_paper_adversarial_before_submit(
        repo_root,
        &mut output,
        &text,
        false,
    );
    router_rs::paper_prose_hook::maybe_merge_paper_prose_before_submit(
        repo_root,
        &mut output,
        &text,
        false,
    );
    let persisted_after_followup = if needs_goal_pre {
    ...
    if !persisted || !persisted_after_followup {
        if gate_needs_persist
            && !router_rs::router_env_flags::router_rs_cursor_hook_state_fail_open_enabled()
        {
            release_state_lock(&mut lock);
            let mut out = json!({ "continue": false });
...
fn handle_subagent_start(repo_root: &Path, event: &Value) -> Value {
    ...
    let independent_fork_pre_goal =
        router_rs::review_gate_engine::cursor_review_independent_fork(fork, pre_goal_kind);
    let independent_fork_review =
        router_rs::review_gate_engine::cursor_review_independent_fork(fork, review_kind);
    ...
    if tool_name_is_rust_file_write_tool(name) {
        if let Some(path) = payload_tool_path(event) {
            if path.extension().and_then(|e| e.to_str()) == Some("rs")
                && path.is_file()
                && router_rs::path_guard::path_is_within_repo_root(repo_root, &path)
            {
                return true;
            }
        }
...
fn handle_post_tool_use(repo_root: &Path, event: &Value) -> Value {
    ...
    if review_armed_peek {
        ...
        if let Err(e) = router_rs::session_call_tracker::record_tool_call(repo_root, &name) {
...
fn handle_post_tool_use_with_lock(
    ...
    let syn = router_rs::hook_posttool_normalize::synthetic_post_tool_evidence_shape(event);
    if let Err(err) = router_rs::framework_runtime::try_append_post_tool_shell_evidence(
        repo_root,
        &syn,
...
fn maybe_run_cursor_rust_lint(repo_root: &Path, event: &Value) -> Option<String> {
    if !router_rs::router_env_flags::router_rs_cursor_cargo_check_sync_enabled() {
        return None;
    }
    ...
    if !router_rs::path_guard::path_is_within_repo_root(repo_root, &path) {
        return None;
    }
    if path.extension().and_then(|e| e.to_str()) != Some("rs") {
        return None;
    ...
    if !router_rs::path_guard::path_is_within_repo_root(repo_root, &cargo_dir) {
        return None;
    }
    ...
    router_rs::hook_timing::add_cargo_check_ms(cargo_start.elapsed().as_millis() as u64);
    ...
    let _ = router_rs::framework_runtime::framework_hook_evidence_append(json!({
...
fn handle_after_agent_response(repo_root: &Path, event: &Value) -> Value {
    ...
    let tail = router_rs::hook_common::hook_assistant_tail_window(
        &text,
        router_rs::hook_common::CURSOR_HOOK_SIGNAL_ASSISTANT_TAIL_CHARS,
    );
...
fn handle_stop(repo_root: &Path, event: &Value) -> Value {
    ...
    if cursor_review_gate_suppressed(repo_root, &stop_prompt_for_profile) {
        ...
        if let Some(nudge) = router_rs::framework_runtime::rfv_nudge_for_key_task(repo_root) {
            out["rfv_suggestion"] = Value::String(nudge);
        }
    ...
    let response_for_lint = router_rs::hook_common::hook_assistant_tail_window(
        &response_full,
        router_rs::hook_common::CURSOR_HOOK_SIGNAL_ASSISTANT_TAIL_CHARS,
    );
    ...
    let (mut output, skip_review_output_lint) = match loaded {
        ...
        Ok(Some(mut state)) => {
            ...
            if review_stop_followup_needed(&state) {
                ...
                let cap =
                    router_rs::router_env_flags::router_rs_cursor_review_gate_stop_max_nudges_cap();
                ...
                let _ = save_state(repo_root, event, &mut state);
                // Stop 只给短码，避免把整段 Goal 契约说明塞进会话收尾（细则见 beforeSubmit / AGENTS）。
    ...
    // I6: 关键任务 RFV 默认启用时注入自动续跑提示
    if let Some(nudge) = router_rs::framework_runtime::rfv_nudge_for_key_task(repo_root) {
        output["rfv_suggestion"] = Value::String(nudge);
    }
...
fn truncate_cursor_sessionstart_context(text: &str) -> String {
    let max_bytes = router_rs::router_env_flags::router_rs_cursor_sessionstart_context_max_bytes();
...
fn handle_session_start(repo_root: &Path, event: &Value) -> Value {
    ...
    if let Err(e) = router_rs::session_call_tracker::init_tracker(repo_root) {
...
fn handle_before_shell_execution(repo_root: &Path, event: &Value) -> Value {
    ...
    let cmd_norm = shell_event_command(event)
        ...
        .unwrap_or_default();
    if !cmd_norm.is_empty() {
        let mut ctx = router_rs::hook_common::path_guard::PathGuardContext::from_repo(repo_root);
        if let Some(dir) =
            router_rs::hook_common::path_guard::resolve_active_skill_dir_from_prompt(
                repo_root,
                &prompt_text(event),
            )
        {
            ctx = ctx.with_active_skill_dir(Some(dir));
        }
        if let Some(reason) =
            router_rs::hook_common::path_guard::cursor_shell_protected_path_deny_reason(
                &cmd_norm, &ctx,
            )
        {
            return json!({
...
fn handle_after_file_edit(repo_root: &Path, event: &Value) -> Value {
    ...
    let rel = router_rs::hook_common::path_guard::relative_candidate_path(path, Some(repo_root));
    if router_rs::hook_common::path_guard::protected_path_block_reason_with_context(&rel, &ctx)
        .is_some()
    {
        return json!({
            "user_message": format!(
                "Edited protected path {rel}; regenerate through the framework runtime instead."
            ),
        });
    }
    ...
    if !router_rs::path_guard::path_is_within_repo_root(repo_root, &p) {
        return json!({});
    }
...
fn handle_session_end(repo_root: &Path, event: &Value) -> Value {
    ...
    if router_rs::router_env_flags::router_rs_cursor_hook_state_legacy_full_sweep_enabled() {
...
fn hook_state_lock_removable_for_sweep(lock_path: &Path) -> bool {
    ...
    let days = router_rs::router_env_flags::router_rs_cursor_hook_state_stale_sweep_days();
    if days > 0 {
...
pub fn parse_terminal_header(text: &str) -> Option<TerminalHeader> {
...
fn collect_terminal_observations(terminals_dir: &Path) -> Vec<TerminalObservation> {
    super::terminal_observation_cache::collect_terminal_observations_cached(
        terminals_dir,
    )
}
...
pub fn normalize_cursor_dispatch_event(event_name: &str) -> String {
    event_name
        .trim()
        .to_ascii_lowercase()
        .chars()
...
pub fn dispatch_cursor_hook_event(
    ...
    let lowered = normalize_cursor_dispatch_event(event_name);
