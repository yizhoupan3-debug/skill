...
pub(super) fn handle_codex_stop(repo_root: &Path, event: &Value) -> Option<Value> {
    ...
    // Suppression check (shared)
    if hook_dispatch::is_review_gate_suppressed("codex", Some(repo_root), &prompt_text) {
        if let Some(msg) = hooks::closeout_stop_followup_for_completion_text(repo_root, &closeout_completion_text(event)) {
            return Some(json!({ "decision": "block", "followup_message": msg }));
        }
        super::reset_hook_state(repo_root, event);
        return None;
    }

    // Closeout advisory (shared)
    if let Some(msg) = hooks::closeout_stop_followup_for_completion_text(repo_root, &closeout_completion_text(event)) {
        return Some(json!({ "decision": "block", "followup_message": msg }));
    }

    // Load state + core decision (shared via hook_dispatch::evaluate_stop_decision)
    match load_state(repo_root, event) {
        Err(reason) => {
            eprintln!("[router-rs] codex hook-state unreadable: {reason}");
            Some(json!({
                "decision": "block",
                "followup_message": format!("router-rs {} need=repair_hook_state_json_or_permissions", lifecycle_host().hook_state_unreadable_tag())
            }))
        }
        Ok(Some(mut state)) => {
            let persist = with_codex_state_lock(repo_root, event, |_loaded| {
                if has_override(&prompt_text) { state.review_gate.review_override = true; }
                if core_policy::hook_common::saw_reject_reason(&stop_signal, &prompt_text) { state.review_gate.reject_reason_seen = true; }
                let goal_entry = core_policy::hook_common::is_framework_goal_entry_prompt(&prompt_text);
                hook_dispatch::update_goal_gate(&mut state.review_gate, &prompt_text, &response_full, goal_entry);
                let assistant_tail = core_policy::hook_common::hook_assistant_tail_window(&response_full, core_policy::hook_common::HOOK_SIGNAL_ASSISTANT_TAIL_CHARS);
                if let Some(phase) = maybe_bump_review_phase_for_compact_findings(state.review_gate.review_required, state.review_gate.review_override, state.phase, state.subagent_start_count, state.review_gate.independent_reviewer_seen, &assistant_tail) {
                    state.phase = phase;
                }
                Ok((Some(state), ()))
            });
            ...
            // Re-load state for decision evaluation
            if let Ok(Some(mut state)) = load_state(repo_root, event) {
                match hook_dispatch::evaluate_stop_decision(
                    &mut state.review_gate, &prompt_text, &response_full,
                    &format!("{prompt_text}\n{response_full}"),
                    &response_full, repo_root, "codex",
                ) {
                    hook_dispatch::StopDecision::Closeout { message }
                    | hook_dispatch::StopDecision::ReviewGateNudge { message } => return Some(json!({ "decision": "block", "followup_message": message })),
                    hook_dispatch::StopDecision::GoalFollowup { message } => return Some(json!({ "decision": "block", "followup_message": message })),
...
fn attach_codex_hook_observation(mut value: Option<Value>) -> Option<Value> {
    if let Some(ref mut v) = value {
        hooks::attach_router_rs_observation(v, "codex");
    }
    value
}
