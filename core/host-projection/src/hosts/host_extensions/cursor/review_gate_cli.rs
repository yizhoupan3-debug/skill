...
pub fn run_cursor_review_gate(event: &str, cli_repo_root: Option<&Path>) -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        let payload = match super::read_cursor_hook_stdin_json() {
            Ok(v) => v,
        ...
        let repo_root =
            super::resolve_cursor_hook_repo_root(cli_repo_root, &payload)?;
        let _registry_guard = core_policy::registry_review_gate::HookRegistryRepoGuard::new(&repo_root);
        let mut output =
            super::dispatch_cursor_hook_event(&repo_root, event, &payload);
        ...
        super::apply_cursor_hook_output_policy(&mut output);
        super::apply_cursor_hook_silent_policy(&mut output);
        let mut stdout = std::io::stdout();
