...
pub fn generated_artifacts_status_for_repo(repo_root: &Path) -> FrameworkResult<Value> {
...
pub fn generated_artifacts_status(
    ...
    let manifest: GeneratedArtifactsManifest = serde_json::from_value(manifest)
        .map_err(|err| FrameworkError::other(format!("invalid generated artifact manifest: {err}")))?;
    ...
    match compare {
        ...
        "normalized-text" => {
            ...
            let checked_in = std::str::from_utf8(checked_in)
                .map_err(|e| FrameworkError::other(format!("normalized-text artifact is not UTF-8: {e}")))?;
        ...
        other => Err(FrameworkError::other(format!(
...
pub fn prepare_generated_artifact_temp_root(
    framework_root: &Path,
    artifact_root: &Path,
) -> FrameworkResult<GeneratedArtifactTempRoot> {
...
pub fn copy_framework_tree_for_generation(source: &Path, destination: &Path) -> FrameworkResult<()> {
    fs::create_dir_all(destination)
        .map_err(|e| FrameworkError::other(format!("failed to create {}: {e}", destination.display())))?;
    for entry in fs::read_dir(source)
        .map_err(|e| FrameworkError::other(format!("failed to read directory {}: {e}", source.display())))?
    {
        let entry = entry.map_err(|e| FrameworkError::other(format!(
        ...
        let metadata = fs::symlink_metadata(&path)
            .map_err(|e| FrameworkError::other(format!("failed to inspect {}: {e}", path.display())))?;
        ...
        if metadata.is_dir() {
            ...
            if let Some(parent) = target.parent() {
                fs::create_dir_all(parent)
                    .map_err(|e| FrameworkError::other(format!("failed to create {}: {e}", parent.display())))?;
            }
            fs::copy(&path, &target).map_err(|e| FrameworkError::other(format!(
                    "failed to copy {} to {}: {e}",
                    path.display(),
                    target.display()
                )))?;
...
pub fn run_generated_artifact_generator(
    ...
    let stdout_file = fs::File::create(&stdout_path)
        .map_err(|e| FrameworkError::other(format!("failed to create {}: {e}", stdout_path.display())))?;
    let stderr_file = fs::File::create(&stderr_path)
        .map_err(|e| FrameworkError::other(format!("failed to create {}: {e}", stderr_path.display())))?;
...
pub fn undeclared_generated_framework_artifacts(
    ...
    for path in candidates {
        let rel = path
            ...
            .map_err(|e| FrameworkError::other(e.to_string()))?
            .to_string_lossy()
            .into_owned();
...
pub fn surface_policy_generated_reports(framework_root: &Path) -> FrameworkResult<BTreeSet<String>> {
...
pub fn skills_source_rel(repo_root: &Path) -> FrameworkResult<String> {
...
pub fn validate_source_rel(source_rel: &str) -> FrameworkResult<()> {
    let candidate = Path::new(source_rel);
    if candidate.as_os_str().is_empty() {
        return Err(FrameworkError::other("skills source_rel must not be empty"));
    }
...
pub fn resolve_router_rs_executable(repo_root: &Path) -> FrameworkResult<PathBuf> {
...
pub fn run_router_rs_json(repo_root: &Path, args: &[String]) -> FrameworkResult<Value> {
    ...
    if output.status.success() {
        let stdout = String::from_utf8(output.stdout).map_err(|e| FrameworkError::other(e.to_string()))?;
        return serde_json::from_str(stdout.trim()).map_err(|e| FrameworkError::other(e.to_string()));
    }
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    Err(FrameworkError::other(if stderr.is_empty() {
        format!(
