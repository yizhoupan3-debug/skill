...
pub fn registry_managed_mcp_server_ids(framework_root: &Path) -> Result<Vec<String>> {
    let registry = framework_core::runtime_registry::load_runtime_registry_json(framework_root)?;
    Ok(registry
                ...
                Ok(()) => {
                    ...
                    if server_id.as_str() == "router-rs-framework" && cmd != "cargo" {
                        ...
                        match resolved.and_then(|path| {
                            framework_core::router_self::validate_router_rs_binary_runnable(&path)
                                .ok()
                        }) {
                            Some(()) => {
                                server_status
...
pub fn install_native_integration(
    ...
    let home_config_dir = home_config_path
        ...
        .unwrap_or_else(|| {
            default_home_dir().join(framework_core::runtime_registry::host_private_config_dir(
                host_id,
            ))
        });
...
pub fn projection_status_command(command: ProjectionStatusCommand) -> Result<Value> {
    ...
    let mut results = Map::new();
    for tool in framework_core::framework_host_targets::skills_install_tools_ordered(
...
pub fn projection_envelope(
    ...
    let pairs = framework_core::framework_host_targets::host_id_and_skills_install_tool_pairs(
        &roots.framework_root,
    )?;
    let registry =
        framework_core::runtime_registry::load_runtime_registry_json(&roots.framework_root)?;
    let mut host_targets_map = serde_json::Map::new();
    for (host_id, tool) in &pairs {
        let value =
            if framework_core::framework_host_targets::host_is_installable(&registry, host_id)? {
...
pub fn registry_projection_tools(framework_root: &Path) -> Result<Vec<String>> {
    ...
    let registry = framework_core::runtime_registry::load_runtime_registry_json(framework_root)?;
    framework_core::framework_host_targets::validate_host_providers_against_registry(&registry)?;
    Ok(tools)
}
...
pub fn ensure_research_mcp_toml(roots: &ResolvedProjectionRoots, host_id: &str) -> Result<bool> {
    let rel = framework_core::runtime_registry::host_projection_mcp_relative(host_id, "project");
