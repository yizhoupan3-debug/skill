...
//!
//! Delegates to [`hook_layer::hooks`] for shared function pointer slots.
//! Only host-projection-specific slots live here.

use serde_json::Value;
use std::path::Path;
use std::sync::OnceLock;

pub use hook_layer::hooks::*;

// ────────────────────────────────────────────────────────────────
// Unique: RESEARCH_TOOL_DISPATCH
// ────────────────────────────────────────────────────────────────
...
// ── Wraps hook-layer's kernel bootstrap + #[cfg(test)] install_test_deps ──

/// Wraps hook-layer's kernel bootstrap + `#[cfg(test)] install_test_deps`.
pub fn ensure_kernel_bootstrap() {
    hook_layer::hooks::ensure_kernel_bootstrap_registered();
    #[cfg(test)]
    install_test_deps();
}
