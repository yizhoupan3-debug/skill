...
pub fn route_task_with_manifest_fallback(
    ...
    if let Some(decision) = literal_framework_alias_decision(&scoped_runtime, query, session_id) {
        crate::hooks::hooks().emit_route_decision(query, &serde_json::to_value(&decision).unwrap_or_default(), false);
        return Ok(decision);
    }
    ...
    let Some(fallback_path) = manifest_fallback_path(runtime_path, manifest_path)? else {
        crate::hooks::hooks().emit_route_decision(query, &serde_json::to_value(&hot_decision).unwrap_or_default(), false);
        ...
        .and_then(|records| routing_engine::route::filter_records_for_host(records, host_id))
    {
        ...
        Err(err) if !should_retry && manifest_path.is_none() => {
            ...
            degraded
             ...
             crate::hooks::hooks().emit_route_decision(query, &serde_json::to_value(&degraded).unwrap_or_default(), false);
            return Ok(degraded);
        }
    ...
    if should_accept_manifest_fallback(
        ...
        crate::hooks::hooks().emit_route_decision(query, &serde_json::to_value(&full_decision).unwrap_or_default(), reroute);
