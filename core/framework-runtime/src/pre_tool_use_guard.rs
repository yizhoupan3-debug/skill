...
pub fn host_requires_strict_pre_tool_fallback(
    ...
    if let Some(strict) = crate::hooks::hooks().host_provider_strict_pre_tool_fallback_hint(id) {
        return Ok(strict);
    }
...
pub fn evaluate_pre_tool_use_guard(
    ...
    crate::hooks::hooks().ensure_kernel_bootstrap();
    ...
    if phase == "approve" {
        ...
        crate::hooks::hooks().emit_tool_call(
            &tool_name,
            0,
            approved && verdict != PreToolUseGuardVerdict::Block,
        );
        crate::hooks::hooks().emit_hook_fired(
            "pre_tool_use_guard",
    ...
    crate::hooks::hooks().emit_hook_fired("pre_tool_use_guard", action);
    Ok(build_response(
        &host_id,
        &tool_name,
        true,
