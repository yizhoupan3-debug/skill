...
use crate::json_io::{read_json_strict, read_text_if_exists};
...
fn resolve_session_repo_root_for_task_ledger(payload: &Value) -> Result<Option<PathBuf>, String> {
    ...
    Ok(Some(crate::repo_roots::resolve_repo_root_arg(Some(path.as_path()))?))
}

pub fn write_framework_session_artifacts(payload: Value) -> Result<Value, String> {
    let run = || -> Result<Value, String> {
        let closeout_evaluation = crate::closeout::enforce_closeout_for_session_payload(&payload)?;
        ...
        if let Some(ref root) = sync_repo
            && let Ok(resolved) = crate::repo_roots::resolve_repo_root_arg(Some(root.as_path())) {
...
fn write_focused_repo_mirrors(
    ...
    let mut registry_rows = crate::util::registry_rows_from_payload(&existing_registry);
    let mut found = false;
    ...
    let (normalized_registry, _, _) =
        crate::util::normalize_task_registry_rows(plan.task_id.clone(), registry_rows);
...
fn build_session_supervisor_state_payload(input: SupervisorStateInput<'_>) -> Value {
    ...
    let evidence_rows = crate::evidence::normalize_evidence_index(input.evidence_payload);
...
fn normalized_continuity(existing: Option<&Value>, status: &str) -> Value {
    let mut payload = existing
        ...
        .cloned()
        .unwrap_or_default();
    let terminal = crate::util::is_terminal(status, crate::constants::TERMINAL_VERIFICATION_STATUSES)
        || crate::util::is_terminal(status, crate::constants::TERMINAL_STORY_STATES);
    payload.insert(
