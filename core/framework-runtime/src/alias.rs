...
use crate::types::FrameworkAliasBuildOptions;
use crate::json_value::{stable_line_items, value_string_list, value_text};
use crate::runtime_view::{classify_runtime_continuity, load_framework_runtime_view, workspace_name_from_root};
...
fn canonical_framework_host_id(host_id: &str) -> &str {
    crate::hooks::hooks()
        .host_provider_for_routing_spelling(Some(host_id))
        .unwrap_or(host_id)
}
...
fn resolve_alias_host_entrypoint(alias_record: &Value, host_id: Option<&str>) -> String {
    let requested_host = canonical_framework_host_id(
        host_id
            ...
            .unwrap_or_else(|| crate::hooks::hooks().default_host_id()),
    );
    ...
    for entry in crate::hooks::hooks().host_provider_registry() {
        let hid = entry.0;
