...
/// 实际规则下沉到 [`core_state::state_manager::evidence_index_entry_implies_success`]
...
fn handle_append_round(payload: &Value, repo_root: &Path) -> Result<Value, String> {
    ...
    let close_goal_text: Option<String> = if loop_status == "closed" {
        let goal_text = obj.get("goal").and_then(Value::as_str).unwrap_or("").to_string();
        Some(goal_text)
    } else {
        None
    };
    let close_rounds_final: usize = if loop_status == "closed" {
        obj.get("rounds").and_then(Value::as_array).map(|a| a.len()).unwrap_or(0)
    } else {
        0
    };
    ...
    if let Some(goal_text) = close_goal_text {
        if let Err(e) = crate::verification_gate::append_quality_gate_evidence(
    ...
    crate::hooks::hooks().emit_quality_gate_round(round_n as u32, &verify_result);
    Ok(resp)
}
