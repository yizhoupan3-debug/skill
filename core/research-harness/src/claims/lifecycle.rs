...
fn merge_string_array(existing: &Value, additions: &[String]) -> Value {
    let mut merged: Vec<String> = existing
        .as_array()
        .map(|items| items.iter().filter_map(Value::as_str).filter(|s| !s.is_empty()).map(ToString::to_string).collect())
        .unwrap_or_default();
...
fn novelty_arr<'a>(state: &'a Value, key: &str) -> &'a [Value] {
    novelty_gate(state).get(key).and_then(Value::as_array).map(|a| a.as_slice()).unwrap_or(&[])
}
...
pub fn sort_entries_by_recency(entries: &[Value], timestamp_field: &str) -> Vec<Value> {
    ...
    sorted.sort_by(|a, b| {
        let ta = parse_iso_timestamp(str_field(a, timestamp_field)).unwrap_or(DateTime::<Utc>::MIN_UTC);
        let tb = parse_iso_timestamp(str_field(b, timestamp_field)).unwrap_or(DateTime::<Utc>::MIN_UTC);
        tb.cmp(&ta)
    });
    sorted
}
...
pub fn current_context_runs(state: &Value) -> Vec<Value> {
    ...
    sort_entries_by_recency(runs, "recorded_at").into_iter().take(FALLBACK_ACTIVITY_LIMIT).collect()
}
...
pub fn find_hypothesis<'a>(state: &'a Value, hypothesis_id: &str) -> Option<&'a Value> {
    arr(state, "hypotheses").iter().find(|h| h.get("id").and_then(Value::as_str) == Some(hypothesis_id))
}
...
pub fn transition_hypothesis(state: &mut Value, index: usize, new_status: &str, reason: Option<&str>) -> Result<()> {
    let hypotheses = arr_mut(state, "hypotheses");
    let h = hypotheses.get_mut(index).ok_or_else(|| anyhow!("Missing hypothesis index"))?;
    let previous = h.get("status").and_then(Value::as_str);
    let allowed = match previous {
        None => vec!["queued", "active", "needs_reflection", "parked", "concluded"],
    ...
    if !allowed.contains(&new_status) {
        bail!("Invalid hypothesis transition: {} -> {}", previous.unwrap_or("none"), new_status);
    }
    let hid = str_field(h, "id").to_string();
    let obj = h.as_object_mut().ok_or_else(|| anyhow!("hypothesis must be object"))?;
    obj.insert("status".into(), json!(new_status));
    obj.insert("status_reason".into(), reason.map(Value::from).unwrap_or(Value::Null));
    obj.insert("status_updated_at".into(), json!(now_iso()));
    let backlog = arr_mut(state, "hypothesis_backlog");
    if new_status == "queued" {
        if !backlog.iter().any(|v| v.as_str() == Some(&hid)) { backlog.push(json!(hid)); }
...
pub fn add_hypothesis(state: &Value, input: HypothesisInput<'_>) -> Result<Value> {
    ...
    let entry = json!({
        ...
        "status_updated_at": now_iso(), "created_at": now_iso(),
    });
...
pub fn record_run(state: &Value, input: &RecordRunInput<'_>, _workspace: &Path) -> Result<Value> {
    ...
    if gate_status != "passed" {
        if !input.override_novelty_gate { bail!("Novelty gate must pass before recording runs (current: {gate_status})"); }
        if input.override_reason.unwrap_or("").trim().is_empty() { bail!("Novelty gate override requires --override-reason"); }
    }
    let current_status = find_hypothesis(&next, input.hypothesis_id).and_then(|h| h.get("status")).and_then(Value::as_str).unwrap_or("queued").to_string();
    if !["active", "queued"].contains(&current_status.as_str()) {
        bail!("Hypothesis {} must be active or queued, current: {current_status}", input.hypothesis_id);
    }
    if current_status == "queued" {
        transition_hypothesis(&mut next, index, "active", Some("activated by first recorded run"))?;
    }
    ...
    transition_hypothesis(&mut next, index, "needs_reflection", Some(&format!("{run_id} recorded")))?;
    Ok(next)
}
...
pub fn annotate_run(state: &Value, run_id: &str, input: RunAnnotationInput<'_>) -> Result<Value> {
    ...
    obj.insert("applies_to".into(), merge_string_array(obj.get("applies_to").unwrap_or(&Value::Null), input.applies_to));
    obj.insert("does_not_apply_to".into(), merge_string_array(obj.get("does_not_apply_to").unwrap_or(&Value::Null), input.does_not_apply_to));
    obj.insert("reuse_annotated_at".into(), json!(now_iso()));
    Ok(next)
}

// ── 反思 ──

pub fn latest_decision_for_hypothesis<'a>(state: &'a Value, hypothesis_id: &str) -> Option<&'a Value> {
    arr(state, "decisions").iter().rev().find(|d| d.get("hypothesis_id").and_then(Value::as_str) == Some(hypothesis_id))
}
...
pub fn reflect(
    ...
    let latest_run = latest_run_for_hypothesis(&next, hypothesis_id).ok_or_else(|| anyhow!("Cannot reflect without a recorded run"))?;
    if let Some(latest_decision) = latest_decision_for_hypothesis(&next, hypothesis_id) {
        if latest_decision.get("run_id") == latest_run.get("run_id") {
            bail!("Run {} already has a reflection", str_field(latest_run, "run_id"));
        }
    }
    ...
    if let Some(target_id) = activate_hypothesis {
        let Some(target_index) = find_hypothesis_index(&next, target_id) else { bail!("Unknown activate_hypothesis: {target_id}"); };
        set_key(&mut next, "active_hypothesis", json!(target_id));
        let target_status = find_hypothesis(&next, target_id).and_then(|h| h.get("status")).and_then(Value::as_str).unwrap_or("-");
        if target_status == "queued" || target_status == "parked" {
            transition_hypothesis(&mut next, target_index, "active", Some("activated after pivot"))?;
        }
...
pub fn recommend_next_actions(state: &Value) -> Vec<String> {
    ...
    if state.get("status").and_then(Value::as_str) == Some("concluded") {
        return vec!["Freeze final narrative in findings.md.".into(), "Archive winning evidence path.".into()];
    }
    ...
    vec![format!("对 {active_id} 做 reflection，选 DEEPEN/BROADEN/PIVOT/CONCLUDE。")]
}
...
pub fn state_freshness(state: &Value) -> StateFreshness {
    ...
    let recent_runs: Vec<Value> = if let Some(active_id) = active_id {
        let filtered: Vec<_> = sort_entries_by_recency(all_runs, "recorded_at")
            .into_iter()
            .filter(|r| {
                r.get("hypothesis_id").and_then(Value::as_str) == Some(active_id)
            })
            .filter(|r| {
    ...
    let recent_decisions: Vec<Value> = if let Some(active_id) = active_id {
        let filtered: Vec<_> = sort_entries_by_recency(all_decisions, "recorded_at")
            .into_iter()
            .filter(|d| {
                d.get("hypothesis_id").and_then(Value::as_str) == Some(active_id)
            })
            .filter(|d| {
...
mod tests {
    ...
    fn add_hypothesis_basic() {
        ...
        let result = add_hypothesis(&state, HypothesisInput {
            claim: "test claim", prediction: Some("p"), mechanism: None, falsifiable_prediction: None,
            success_threshold: None, stop_condition: None, baselines: &[], confounders: &[],
            negative_signals: &[], minimal_test: None, priority: "high", hypothesis_id: Some("h1"),
        }).unwrap();
        assert_eq!(arr(&result, "hypotheses").len(), 1);
        assert_eq!(find_hypothesis(&result, "h1").unwrap().get("claim").and_then(Value::as_str), Some("test claim"));
    }

    #[test]
    fn add_hypothesis_duplicate_rejected() {
        let state = gate_passed_state();
        let s = add_hypothesis(&state, HypothesisInput {
            claim: "c", prediction: None, mechanism: None, falsifiable_prediction: None,
            success_threshold: None, stop_condition: None, baselines: &[], confounders: &[],
            negative_signals: &[], minimal_test: None, priority: "medium", hypothesis_id: Some("dup"),
        }).unwrap();
        let result = add_hypothesis(&s, HypothesisInput {
            claim: "c2", prediction: None, mechanism: None, falsifiable_prediction: None,
            success_threshold: None, stop_condition: None, baselines: &[], confounders: &[],
            negative_signals: &[], minimal_test: None, priority: "medium", hypothesis_id: Some("dup"),
        });
    ...
    fn transition_hypothesis_valid() {
        ...
        arr_mut(&mut s, "hypotheses").push(json!({"id": "h1", "claim": "c", "status": "queued", "status_updated_at": now_iso(), "created_at": now_iso()}));
        let idx = find_hypothesis_index(&s, "h1").unwrap();
        transition_hypothesis(&mut s, idx, "active", Some("activated")).unwrap();
        assert_eq!(find_hypothesis(&s, "h1").unwrap().get("status").and_then(Value::as_str), Some("active"));
    }

    #[test]
    fn transition_hypothesis_invalid() {
        let mut s = gate_passed_state();
        arr_mut(&mut s, "hypotheses").push(json!({"id": "h1", "claim": "c", "status": "concluded", "status_updated_at": now_iso(), "created_at": now_iso()}));
        let idx = find_hypothesis_index(&s, "h1").unwrap();
    ...
    fn record_run_basic() {
        ...
        let s2 = add_hypothesis(&s, HypothesisInput {
            claim: "c", prediction: None, mechanism: None, falsifiable_prediction: None,
            success_threshold: None, stop_condition: None, baselines: &[], confounders: &[],
            negative_signals: &[], minimal_test: None, priority: "medium", hypothesis_id: Some("h1"),
        }).unwrap();
        let tmp = tempfile::tempdir().unwrap();
        let result = record_run(&s2, &RecordRunInput {
            hypothesis_id: "h1", outcome: "confirmatory", summary: "test",
            metric_name: None, metric_value: None, command: None, evidence_path: None,
            sanity_checks: &[], baseline_result: None, rules_in: &[], rules_out: &[],
            alternative_explanations: &[], threats: &[], interpretation: None,
            finding: None, decision_delta: None, reuse_note: None,
            applies_to: &[], does_not_apply_to: &[], override_novelty_gate: false, override_reason: None,
        }, tmp.path()).unwrap();
    ...
    fn record_run_rejects_unknown() {
        let s = gate_passed_state();
        let tmp = tempfile::tempdir().unwrap();
        let result = record_run(&s, &RecordRunInput {
            hypothesis_id: "nonexistent", outcome: "confirmatory", summary: "s",
            metric_name: None, metric_value: None, command: None, evidence_path: None,
            sanity_checks: &[], baseline_result: None, rules_in: &[], rules_out: &[],
            alternative_explanations: &[], threats: &[], interpretation: None,
            finding: None, decision_delta: None, reuse_note: None,
    ...
    fn reflect_transitions_hypothesis() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let s3 = record_run(&s2, &RecordRunInput {
            hypothesis_id: "h1", outcome: "confirmatory", summary: "s",
            metric_name: None, metric_value: None, command: None, evidence_path: None,
            sanity_checks: &[], baseline_result: None, rules_in: &[], rules_out: &[],
            alternative_explanations: &[], threats: &[], interpretation: None,
            finding: None, decision_delta: None, reuse_note: None,
        ...
        assert_eq!(result.get("current_direction").and_then(Value::as_str), Some("DEEPEN"));
        assert_eq!(find_hypothesis(&result, "h1").unwrap().get("status").and_then(Value::as_str), Some("active"));
    }
    ...
    fn reflect_conclude() {
        ...
        assert_eq!(result.get("status").and_then(Value::as_str), Some("concluded"));
    }
