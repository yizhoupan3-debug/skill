...
use crate::util::{
    arr, arr_mut, novelty_gate, novelty_gate_mut, set_key, str_field,
...
pub fn transition_hypothesis(state: &mut Value, index: usize, new_status: &str, reason: Option<&str>) -> Result<()> {
    ...
    obj.insert("status_updated_at".into(), json!(framework_kernel::time::now_iso()));
...
pub fn annotate_run(state: &Value, run_id: &str, input: RunAnnotationInput<'_>) -> Result<Value> {
    ...
    obj.insert("reuse_annotated_at".into(), json!(framework_kernel::time::now_iso()));
    Ok(next)
}
