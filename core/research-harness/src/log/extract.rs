...
pub static DEFAULT_PATTERNS: LazyLock<ExtractionPatterns> = LazyLock::new(|| ExtractionPatterns {
    method: {
...
mod tests {
    ...
    fn extract_method_entities() {
        ...
        let methods: Vec<_> = entities
            ...
            .collect();
        assert!(
            methods.contains(&"Transformer"),
            "should find Transformer as method"
        );
    }
    ...
    fn extract_dataset_entities() {
        ...
        let datasets: Vec<_> = entities
            .iter()
            .filter(|(_, k)| k == "dataset")
            .map(|(n, _)| n.as_str())
            .collect();
