...
use crate::latex::functions::{self, ArgType, FunctionContext, FUNCTIONS};
use crate::latex::macro_expander::{MacroExpander, IMPLICIT_COMMANDS};
use crate::latex::parse_node::{AtomFamily, Mode, ParseNode};
...
impl<'a> Parser<'a> {
    ...
    pub fn parse_atom(
        ...
        loop {
            ...
            if lex.text == "\\limits" || lex.text == "\\nolimits" {
                ...
                .chars()
                .next()
                .and_then(crate::latex::unicode_sup_sub::unicode_sub_sup)
            {
            ...
            Some(ArgType::HBox) => {
                ...
                match group {
                    Some(g) => Ok(Some(ParseNode::Styling {
                        mode: g.mode(),
                        style: crate::latex::parse_node::StyleStr::Text,
                        body: vec![g],
                        loc: None,
                    })),
                    None => Ok(None),
    ...
    pub fn parse_size_group(&mut self, optional: bool) -> ParseResult<Option<ParseNode>> {
        ...
        Ok(Some(ParseNode::Size {
            mode: self.mode,
            value: crate::latex::parse_node::Measurement {
                number,
                unit: unit.to_string(),
            },
            is_blank,
