...
use crate::latex::macro_expander::MacroDefinition;
use crate::latex::parse_node::{
    AlignSpec, AlignType, ArrayTag, Measurement, Mode, ParseNode, ProofBranch, ProofLineStyle,
    StyleStr,
};
use crate::latex::parser::Parser;
...
pub fn parse_array(
    ...
    loop {
        ...
        if next == "&" {
            ...
            let is_empty_trailing = if let Some(s) = style {
                if s == StyleStr::Text || s == StyleStr::Display {
                    if let ParseNode::Styling { body: ref sb, .. } = cell {
                        if let Some(ParseNode::OrdGroup { body: ob, .. }) = sb.first() {
                            ob.is_empty()
                        } else {
                            false
                        }
            ...
            } else if let ParseNode::OrdGroup { body: ob, .. } = cell {
                ob.is_empty()
            } else {
                false
            };
