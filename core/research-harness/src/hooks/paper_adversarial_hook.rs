...
mod tests {
    ...
    // ── Tests from existing research-harness paper_adversarial.rs ──
    ...
    #[test]
    fn weak_signals_need_five_hits_from_legacy() {
    ...
    #[test]
    fn ml_tech_discussion_suppressed_legacy() {
        assert!(maybe_append_adversarial_context(
            "The training loss uses a transformer architecture with layer normalization and attention"
        )
        .is_none());
    ...
    #[test]
    fn ml_tech_with_paper_keyword_not_suppressed_legacy() {
        assert!(maybe_append_adversarial_context(
            "请根据审稿意见修改这篇手稿的 baseline 和 ablation 实验设计"
        )
        .is_some());
