...
//! ```ignore
//! mcp_tool_registry::browser_dispatch::set_hooks(BrowserMcpHooks {
//!     evaluate_mcp_pre_guard: |tool, args, repo| {
//!         let v = host_projection::hooks::evaluate_mcp_pre_guard_safe(tool, args, repo);
//!         mcp_tool_registry::browser_dispatch::McpPreGuardVerdict { blocked: v.blocked, reason: v.reason }
//!     },
...
//!
//! ## Lint
//! This module denies unwrap/expect in non-test code. The explicit `.expect()` panic on
...
pub fn hooks() -> &'static BrowserMcpHooks {
    ...
    HOOKS.get().expect(
        "BrowserMcpHooks not initialized — call mcp_tool_registry::browser_dispatch::set_hooks() before using browser-mcp",
    )
}
