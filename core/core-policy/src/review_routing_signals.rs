...
pub fn review_gate_compiled_regexes() -> &'static [Regex] {
    &loaded().review_gate_regexes
}

pub struct ParallelReviewCandidateMarkers {
    pub review_markers: &'static [String],
    pub breadth_markers: &'static [String],
    pub scope_markers: &'static [String],
}

pub fn parallel_review_candidate_markers() -> ParallelReviewCandidateMarkers {
    let l = loaded();
...
mod tests {
    ...
    fn embedded_review_routing_json_parses() {
        ...
        assert!(
            parsed.review_gate_regexes.len() >= 17,
            "expected at least 17 review gate regexes, got {}",
