...
const ZH_IMPLEMENT_VERBS: &[&str] = &[
    "实现", "重构", "添加", "添加新", "修改", "编写", "构建", "迁移",
    "优化", "修复", "设计", "创建", "整合", "改造", "升级", "扩展",
    "调优", "开发", "搭建", "建立", "实现一个", "写一个", "做一个",
];

/// English implementation verbs.
const EN_IMPLEMENT_VERBS: &[&str] = &[
    "implement", "refactor", "add", "create", "build", "write",
    "migrate", "optimize", "fix", "design", "develop", "integrate",
    "upgrade", "extend", "rework",
];

/// Common file extensions that signal code references.
const CODE_EXTENSIONS: &[&str] = &[
    ".rs", ".ts", ".tsx", ".js", ".jsx", ".py", ".go", ".java",
    ".kt", ".swift", ".c", ".cpp", ".h", ".hpp", ".css", ".scss",
    ".json", ".yaml", ".toml", ".md", ".sql",
];

/// Keywords for scope / requirement changes (amend detection).
const SCOPE_CHANGE_KEYWORDS_ZH: &[&str] = &[
    "增加", "修改", "补充", "额外", "调整", "变化", "新需求", "再加",
    "扩展", "改成", "还要", "对了", "等一下", "还有", "但是", "不过",
    "另外", "顺便", "追加", "变更", "改动一下", "换个方式", "重新",
];

const SCOPE_CHANGE_KEYWORDS_EN: &[&str] = &[
    "apart from", "also need", "additionally", "one more thing",
    "actually", "instead", "change", "update", "modify", "revise",
    "add", "extra", "new requirement", "on second thought",
...
fn has_structured_markers(text: &str) -> bool {
    let markers = [
        "Goal:", "Non-goals:", "Done when:", "目标：", "非目标：",
        "完成条件：", "验证命令:", "Validation",
    ];
    let count = markers
        .iter()
...
fn references_multiple_crates_or_modules(text: &str) -> bool {
    ...
    let path_patterns = ["core/", "src/", "lib/", "app/", "components/", "utils/", "services/", "models/"];
    let path_count = path_patterns
        .iter()
        .filter(|p| text.contains(*p))
        .count();
    ...
    let crate_refs = [
        "crate::", "mod ", "use ", "import ", "require(", "from '",
        "from \"", "package ", "namespace ",
    ];
    let ref_count = crate_refs
        .iter()
...
pub fn detect_scope_change(text: &str) -> bool {
    let lower = text.to_ascii_lowercase();
    let zh_hit = SCOPE_CHANGE_KEYWORDS_ZH
        .iter()
        .any(|k| text.contains(k));
    let en_hit = SCOPE_CHANGE_KEYWORDS_EN
        .iter()
...
mod tests {
    ...
    fn complex_implementation_task_zh() {
        let text = "重构 core/state_manager 模块中的 goal_ops.rs，需要拆分 checkpoint 逻辑到单独的文件，
    ...
    fn scope_change_not_detected_for_simple_questions() {
        let cases = ["好的", "继续", "明白了", "下一步", "yes", "ok", "looks good", "继续推进"];
        for case in &cases {
