...
pub struct EvolutionObserver {
    config: EvolutionObserverConfig,
    events_seen: u64,
    counters: WindowCounters,
}

impl EvolutionObserver {
    pub fn new(config: EvolutionObserverConfig) -> Self {
        Self {
            config,
            events_seen: 0,
...
pub struct FanoutTelemetryWriter {
    ...
    observer: Arc<Mutex<EvolutionObserver>>,
}

impl FanoutTelemetryWriter {
    pub fn new(inner: MpscTelemetryWriter, observer: EvolutionObserver) -> Self {
        Self {
            inner,
    ...
    pub fn observer(&self) -> Arc<Mutex<EvolutionObserver>> {
...
mod tests {
    ...
    fn observer_counts_route_and_tool_events() {
        ...
        let mut obs = EvolutionObserver::new(EvolutionObserverConfig {
...
"#,
        ...
        let mut obs = EvolutionObserver::new(EvolutionObserverConfig {
            alerts_path: alerts,
            reroute_rate_alert: 1.0,
            ..EvolutionObserverConfig::default()
        });
        let mut offset = 0u64;
    ...
    fn fanout_writer_forwards_to_inner() {
        ...
        let observer = EvolutionObserver::new(EvolutionObserverConfig {
