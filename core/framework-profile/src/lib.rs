...
mod tests {
    ...
    fn validation_rejects_empty_profile_id() {
        ...
        let error = build_profile_bundle(&profile).expect_err("should reject empty profile_id");
        assert!(error.contains("profile_id"));
    }

    #[test]
    fn validation_rejects_empty_display_name() {
        let mut profile = sample_profile();
        profile.display_name = "  ".to_string();
        let error = build_profile_bundle(&profile).expect_err("should reject empty display_name");
        assert!(error.contains("display_name"));
    }

    #[test]
    fn validation_rejects_empty_framework_profile_version() {
        let mut profile = sample_profile();
        profile.framework_profile_version = "".to_string();
        let error = build_profile_bundle(&profile).expect_err("should reject empty version");
        assert!(error.contains("framework_profile_version"));
    }

    #[test]
    fn validation_rejects_missing_core_capabilities() {
        let mut profile = sample_profile();
        profile.core_capabilities = vec!["runtime".to_string()]; // missing artifact, orchestration
        let error = build_profile_bundle(&profile).expect_err("should reject missing capabilities");
        assert!(error.contains("missing core capabilities"));
    }

    #[test]
    fn mcp_servers_string_gets_server_id() {
        let mut profile = sample_profile();
        profile.mcp_servers = vec![json!("my-server")];
        let bundle = build_profile_bundle(&profile).expect("bundle should build");
        let servers = &bundle.mcp_servers;
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0]["server_id"], json!("my-server"));
    }

    #[test]
    fn mcp_servers_object_preserves_existing_server_id() {
        let mut profile = sample_profile();
        profile.mcp_servers = vec![json!({"server_id": "custom-id", "command": "node"})];
        let bundle = build_profile_bundle(&profile).expect("bundle should build");
        let servers = &bundle.mcp_servers;
        assert_eq!(servers[0]["server_id"], json!("custom-id"));
    }

    #[test]
    fn mcp_servers_object_infers_id_from_id_field() {
        let mut profile = sample_profile();
        profile.mcp_servers = vec![json!({"id": "inferred-id", "command": "node"})];
        let bundle = build_profile_bundle(&profile).expect("bundle should build");
        let servers = &bundle.mcp_servers;
        assert_eq!(servers[0]["server_id"], json!("inferred-id"));
    }

    #[test]
    fn session_policy_default_mode_is_default() {
        let bundle = build_profile_bundle(&sample_profile()).expect("bundle should build");
        let session = bundle.codex_profile.get("session_contract").unwrap();
        assert_eq!(session["mode"], json!("bounded"));
        assert_eq!(session["approval_mode"], json!("manual"));
    }

    #[test]
    fn compile_session_mode_defaults() {
        let policy = Map::new();
        let mode = compile_session_mode(&policy);
        assert_eq!(mode["mode"], json!("default"));
        assert_eq!(mode["approval_mode"], json!("inherit"));
        assert_eq!(mode["history_policy"], json!("host-managed"));
        assert_eq!(mode["takeover"], json!(false));
    }

    #[test]
    fn merge_json_maps_deep_merge() {
        let mut target = Map::new();
        target.insert("a".to_string(), json!({"b": 1, "c": 2}));
        target.insert("d".to_string(), json!([1, 2]));

        let mut override_map = Map::new();
        override_map.insert("a".to_string(), json!({"b": 10, "e": 3}));
        override_map.insert("d".to_string(), json!([2, 3]));

        merge_json_maps(&mut target, &override_map);

        assert_eq!(target["a"]["b"], json!(10));
        assert_eq!(target["a"]["c"], json!(2));
        assert_eq!(target["a"]["e"], json!(3));
        let arr = target["d"].as_array().unwrap();
        assert_eq!(arr.len(), 3); // [1, 2, 3] - deduplicated
    }
    ...
    #[test]
    fn value_object_constructs_json() {
        let obj = value_object([
            ("key1", Value::String("val1".to_string())),
            ("key2", json!(42)),
        ]);
        assert_eq!(obj["key1"], json!("val1"));
        assert_eq!(obj["key2"], json!(42));
    }

    #[test]
    fn build_control_plane_contract_descriptors_has_four_contracts() {
        let descriptors = build_control_plane_contract_descriptors();
        assert_eq!(descriptors.len(), 4);
        assert!(descriptors.contains_key(EXECUTION_PROTOCOL_CONTRACT_ARTIFACT_ID));
        assert!(descriptors.contains_key(EXECUTION_CONTROLLER_CONTRACT_ARTIFACT_ID));
        assert!(descriptors.contains_key(DELEGATION_CONTRACT_ARTIFACT_ID));
        assert!(descriptors.contains_key(SUPERVISOR_STATE_CONTRACT_ARTIFACT_ID));
    }

    #[test]
    fn profile_bundle_codex_profile_has_required_sections() {
        let bundle = build_profile_bundle(&sample_profile()).expect("bundle should build");
        assert!(bundle.codex_profile.get("runtime_surface").is_some());
        assert!(bundle.codex_profile.get("execution_surface").is_some());
        assert!(bundle.codex_profile.get("host_projection").is_some());
        assert!(bundle.codex_profile.get("metadata").is_some());
        assert!(bundle.codex_profile.get("capabilities").is_some());
    }

    #[test]
    fn profile_bundle_full_codex_has_common_contract() {
        let bundle = build_profile_bundle(&sample_profile()).expect("bundle should build");
        assert!(bundle.full_codex_profile.get("common_contract").is_some());
        assert!(bundle.full_codex_profile.get("capability_surface").is_some());
    }
}
