...
pub(crate) fn worker_log_path(state_path: &Path, worker_id: &str) -> PathBuf {
...
pub(crate) fn load_store(path: &Path) -> Result<SessionSupervisorStore, String> {
    if !path.is_file() {
...
pub(crate) fn save_store(path: &Path, store: &SessionSupervisorStore) -> Result<(), String> {
...
pub(crate) fn upsert_worker(store: &mut SessionSupervisorStore, worker: WorkerSessionRecord) {
...
pub(crate) fn resolve_state_path(payload: &Value) -> Result<PathBuf, String> {
...
pub(crate) fn now_from_payload(payload: &Value) -> Result<String, String> {
...
pub(crate) fn add_seconds_rfc3339(now: &str, seconds: i64) -> Result<String, String> {
...
pub(crate) fn ensure_lane_contract_metadata(
    metadata: Value,
    worker_id: &str,
    host: &str,
    cwd: &str,
