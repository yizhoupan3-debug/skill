...
use crate::claude_desktop_hooks::{run_claude_desktop_mcp_loop, run_antigravity_mcp_loop};
...
pub(crate) fn dispatch_router_command(command: RouterCommand) -> Result<(), String> {
    match command {
        ...
        RouterCommand::ClaudeDesktop { command } => dispatch_claude_desktop_command(command),
...
fn dispatch_framework_skills(command: SkillsSubcommand) -> Result<(), String> {
...
/// Unified host command dispatcher (merged: codex, cursor, claude, claude-desktop)
fn dispatch_host_command(command: HostCommand) -> Result<(), String> {
    match command {
        ...
        HostCommand::ClaudeDesktop { command } => dispatch_claude_desktop_command(command),
        HostCommand::Antigravity { command } => dispatch_antigravity_command(command),
        HostCommand::AntigravityCli { command } => dispatch_antigravity_cli_command(command),
...
fn dispatch_diagnose_command(command: DiagnoseCommand) -> Result<(), String> {
    match command {
...
fn dispatch_codex_command(command: CodexSubcommand) -> Result<(), String> {
    match command {
...
fn dispatch_cursor_command(command: CursorSubcommand) -> Result<(), String> {
    match command {
...
fn dispatch_claude_command(command: ClaudeSubcommand) -> Result<(), String> {
    match command {
...
fn dispatch_claude_desktop_command(command: ClaudeDesktopSubcommand) -> Result<(), String> {
    match command {
        ClaudeDesktopSubcommand::Agent(command) => {
            let root = resolve_repo_root_arg(command.repo_root.as_deref())?;
            run_claude_desktop_mcp_loop(Some(&root))
        }
    }
}
fn dispatch_antigravity_command(command: AntigravitySubcommand) -> Result<(), String> {
    match command {
        AntigravitySubcommand::Agent(command) => {
            eprintln!(
                "[router-rs] deprecate: `antigravity agent` → use `host antigravity-app agent` or `antigravity-app agent`"
            );
            let root = resolve_repo_root_arg(command.repo_root.as_deref())?;
...
fn dispatch_opencode_command(command: OpenCodeSubcommand) -> Result<(), String> {
    match command {
...
fn dispatch_antigravity_cli_command(command: AntigravityCliSubcommand) -> Result<(), String> {
    match command {
        AntigravityCliSubcommand::Hook(command) => {
            let repo_root = resolve_repo_root_arg(command.repo_root.as_deref())?;
            let payload = run_antigravity_cli_hook(&command.name, &repo_root)?;
            print_json_value(&codex_hook_stdout_payload(payload))?;
            Ok(())
        }
        AntigravityCliSubcommand::InstallHooks(command) => {
            let resolved_home =
                resolve_antigravity_cli_home(command.antigravity_cli_home.as_deref())?;
            let mode = if command.check {
                InstallMode::Check
            } else {
                InstallMode::Apply
            ...
            let repo_root = command
                .repo_root
                .unwrap_or_else(|| std::env::current_dir().unwrap_or(PathBuf::from(".")));
            let payload = install_antigravity_cli_hooks(&resolved_home, &repo_root, mode)?;
            print_json_value(&payload)
        }
    }
}
...
fn dispatch_browser_command(command: BrowserSubcommand) -> Result<(), String> {
    match command {
...
fn dispatch_profile_command(command: ProfileSubcommand) -> Result<(), String> {
    match command {
...
fn dispatch_schema_drift_command(command: SchemaDriftCommand) -> Result<(), String> {
