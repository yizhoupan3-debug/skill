...
pub(crate) const CLI_THIN_SHELL_LINE_BUDGET: usize = 1500;

/// `runtime_ops.inc` 行数上限（P7 增量切片：全量迁入 B3 前只减不增）。
pub(crate) const RUNTIME_OPS_INC_LINE_CEILING: usize = 3400;

pub(crate) fn cli_thin_shell_line_count() -> usize {
