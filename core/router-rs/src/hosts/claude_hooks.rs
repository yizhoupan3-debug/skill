...
fn claude_review_gate_incomplete_stop_reason() -> String {
    ...
    {
        "router-rs CLAUDE_REVIEW_GATE incomplete: run an observed independent reviewer lane \
         (fork_context=false, or omit fork_context when \
         ROUTER_RS_CLAUDE_REVIEW_FORK_CONTEXT_MISSING_INFER_FALSE=1) before closing this review turn."
            .to_string()
    } else {
...
/// PreToolUse deny: only files truly generated by `framework skills refresh --write-companions` or framework install.
...
/// PostToolUse提醒 + Stop门禁: framework source code that needs contract tests after modification.
...
fn run_stop(repo_root: &Path, payload: &Value) -> Option<Value> {
    ...
    {
        return block_stop(&claude_review_gate_incomplete_stop_reason());
    }
...
mod tests {
    ...
    fn pre_tool_use_allows_lexical_traversal_to_agents_md() {
        ...
        // AGENTS.md is no longer in GENERATED_ENTRYPOINT_PATHS_CLAUDE
