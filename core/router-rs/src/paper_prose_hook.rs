...
pub(crate) const PREFIX_LINE: &str = "**PAPER_PROSE_QUALITY_HOOK**";

const BUILTIN_TXT: &str = include_str!("../../../configs/framework/PAPER_PROSE_QUALITY_HOOK.txt");

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum PaperProseHookHost {
    Cursor,
    Codex,
    Claude,
    AntigravityCli,
...
pub(crate) fn maybe_append_paper_prose_context(
    repo_root: &Path,
    prompt_text: &str,
...
pub(crate) fn maybe_merge_paper_prose_before_submit(
    repo_root: &Path,
    output: &mut Value,
    prompt_text: &str,
