...
pub(crate) fn run_host_integration_payload(cli: Cli) -> Result<Value, String> {
...
pub(crate) fn try_framework_root_from_workspace_env() -> Option<PathBuf> {
...
pub(crate) fn try_framework_root_from_current_exe() -> Option<PathBuf> {
    let exe = std::env::current_exe().ok()?;
    framework_root_from_executable_path(&exe)
}

pub(crate) fn resolve_framework_root(explicit: Option<&Path>) -> Result<PathBuf, String> {
...
pub(crate) fn resolve_projection_framework_root(explicit: Option<&Path>) -> Result<PathBuf, String> {
...
pub(crate) fn resolve_project_root(explicit: Option<&Path>, framework_root: &Path) -> Result<PathBuf, String> {
    if let Some(path) = explicit {
        return normalize_path(path);
    }
...
pub(crate) fn normalize_discovered_project_root(
    candidate: &Path,
    framework_root: &Path,
...
pub(crate) fn resolve_artifact_root(
...
pub(crate) fn is_ephemeral_executable_path(path: &str) -> bool {
    crate::router_self::is_ephemeral_router_rs_path(path)
}

pub(crate) fn is_repo_build_executable_path(path: &str, framework_root: &Path) -> bool {
...
pub(crate) fn resolve_mcp_router_rs_command(framework_root: &Path) -> McpRouterRsCommand {
...
pub(crate) fn mcp_router_rs_command_value(command: &McpRouterRsCommand) -> Value {
    match command {
...
pub(crate) fn ensure_router_rs_installed_for_mcp_with_roots(roots: &ResolvedProjectionRoots) -> Result<(), String> {
    if matches!(
...
pub(crate) fn host_account_home_from_roots(roots: &ResolvedProjectionRoots) -> PathBuf {
...
pub(crate) fn router_rs_cargo_bootstrap_args(framework_root: &Path, host_args: &[&str]) -> Vec<String> {
...
pub(crate) fn resolve_host_home(
    explicit: Option<&Path>,
...
pub(crate) fn nearest_marker_root(start: &Path, marker: &str) -> Option<PathBuf> {
    start
        .ancestors()
