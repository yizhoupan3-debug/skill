...
pub(crate) fn host_provider_manifest_host_ids(registry: &Value) -> Result<Vec<String>, String> {
...
pub(crate) fn validate_host_providers_against_registry(registry: &Value) -> Result<(), String> {
...
mod tests {
    ...
    fn host_provider_mod_declarations_align_with_registry() {
        let root = repo_root();
        let reg = load_runtime_registry_json(&root).expect("registry");
        let hosts_mod = std::fs::read_to_string(root.join("core/router-rs/src/hosts/mod.rs"))
            .expect("hosts/mod.rs");
        let cargo_toml = std::fs::read_to_string(root.join("core/router-rs/Cargo.toml"))
            .expect("Cargo.toml");
