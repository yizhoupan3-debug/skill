//! Test-only helpers for hook-layer: mock registrations, env lock, tokenizer, etc.
//!
...
请在完成态宣称前写入 closeout record 并通过评估。",
        ...
        fn test_append_prose_context(
            ...
            host: hooks::PaperProseHookHost,
        ) {
            if !hooks::router_rs_operator_inject_globally_enabled() {
                return;
            }
            let env_var = match host {
                hooks::PaperProseHookHost::Cursor => "ROUTER_RS_CURSOR_PAPER_PROSE_HOOK",
                hooks::PaperProseHookHost::Codex => "ROUTER_RS_CODEX_PAPER_PROSE_HOOK",
                hooks::PaperProseHookHost::Claude => "ROUTER_RS_CLAUDE_PAPER_PROSE_HOOK",
                hooks::PaperProseHookHost::OpenCode => "ROUTER_RS_OPENCODE_PAPER_PROSE_HOOK",
            };
        ...
        fn test_append_adversarial_context(
            ...
            _host: hooks::PaperProseHookHost,
        ) {
            // No-op for tests.
        }
        ...
        fn test_attach_observation(output: &mut Value, host: hooks::HookObservationHost) {
            let host_str = match host {
                hooks::HookObservationHost::Cursor => "cursor",
                hooks::HookObservationHost::Codex => "codex",
                hooks::HookObservationHost::Claude => "claude",
                hooks::HookObservationHost::OpenCode => "opencode",
            };
