...
fn rule_category(rule: &str) -> &'static str {
    match rule {
        "schema_version_mismatch" | "task_id_missing" | "summary_missing"
        | "verification_status_missing" | "verification_status_invalid"
        | "task_id_context_mismatch" | "parse_error" | "invalid_command_evidence" => "hard",
        "claimed_done_without_evidence" | "changed_files_without_command_or_risk"
        | "verification_passed_with_failed_command" | "verification_passed_with_missing_artifact"
        | "not_run_without_blockers_or_risks" | "claimed_done_with_failed_verification"
        | "claimed_passed_without_evidence" | "claimed_passed_without_evidence_index_rows" => "soft",
        _ => "hard",
    }
}
...
pub fn evaluate_closeout_record_value(payload: Value) -> Result<Value> {
    ...
    let task_id = payload.get("task_id").and_then(Value::as_str).unwrap_or("").to_string();
    ...
    match serde_json::from_value::<CloseoutRecord>(payload) {
        Ok(record) => {
            ...
            if status == "passed"
                ...
                && !has_success
                && !response.violations.iter().any(|v| v.rule == "claimed_passed_without_evidence")
            {
                ...
                response.missing_evidence.push("evidence_index_successful_row".into());
...
impl CloseoutViolation {
    fn new(rule: impl Into<String>, severity: impl Into<String>, detail: impl Into<String>) -> Self {
        let rule = rule.into();
        let category = rule_category(&rule).to_string();
        Self { rule, severity: severity.into(), category, detail: detail.into() }
    }
}
...
fn evaluate_record(record: &CloseoutRecord) -> CloseoutResponse {
    ...
    // Schema version
    if record.schema_version.trim().is_empty() || record.schema_version != CLOSEOUT_RECORD_SCHEMA_VERSION {
        violations.push(CloseoutViolation::new("schema_version_mismatch", "block",
            format!("expected schema_version={CLOSEOUT_RECORD_SCHEMA_VERSION}, got {:?}", record.schema_version)));
    }
    // Task id
    if record.task_id.trim().is_empty() {
        violations.push(CloseoutViolation::new("task_id_missing", "block", "task_id must be non-empty"));
        missing.push("task_id".into());
    }
    // Summary
    let summary_trimmed = record.summary.trim();
    if summary_trimmed.is_empty() {
        violations.push(CloseoutViolation::new("summary_missing", "block", "summary must be non-empty"));
    ...
    if !status_lower.is_empty() && !status_recognized {
        violations.push(CloseoutViolation::new("verification_status_invalid", "block",
            format!("verification_status must be one of {:?}, got {:?}", ALLOWED_VERIFICATION_STATUSES, record.verification_status)));
    }
    if status_lower.is_empty() {
        violations.push(CloseoutViolation::new("verification_status_missing", "block",
    ...
    if claimed_completion && status_lower == "not_run" && record.risks.is_empty() && record.blockers.is_empty() {
        violations.push(CloseoutViolation::new("claimed_done_without_evidence", "block",
    ...
    if !record.changed_files.is_empty() && record.commands_run.is_empty() && record.risks.is_empty() {
        violations.push(CloseoutViolation::new("changed_files_without_command_or_risk", "block",
            format!("{} changed file(s) recorded but no commands_run and no risks declared", record.changed_files.len())));
    ...
    if status_lower == "passed" {
        if let Some(failed) = record.commands_run.iter().find(|c| c.exit_code != 0) {
            violations.push(CloseoutViolation::new("verification_passed_with_failed_command", "block",
                format!("verification_status=passed but command exited {}: {}", failed.exit_code, failed.command)));
        }
    }
    // R3b: Invalid command evidence
    for cmd in &record.commands_run {
        if cmd.command.trim().is_empty() {
            violations.push(CloseoutViolation::new("invalid_command_evidence", "block",
                format!("commands_run contains a row without a non-empty command; exit_code={}", cmd.exit_code)));
            missing.push("command".into());
        }
    }
    // R4: Verification passed with missing artifact
    if status_lower == "passed" {
        if let Some(ma) = record.artifacts_checked.iter().find(|a| !a.exists) {
            violations.push(CloseoutViolation::new("verification_passed_with_missing_artifact", "block",
                format!("verification_status=passed but artifact does not exist: {}", ma.path)));
        }
    }
    // R5: Not run without blockers or risks
    if status_lower == "not_run" && record.blockers.is_empty() && record.risks.is_empty() {
        let already = violations.iter().any(|v| v.rule == "claimed_done_without_evidence");
        if !already {
            violations.push(CloseoutViolation::new("not_run_without_blockers_or_risks", "block",
    ...
    if status_lower == "failed" && claimed_completion && record.risks.is_empty() && record.blockers.is_empty() {
    ...
    if status_lower == "passed" && record.commands_run.is_empty() && record.artifacts_checked.is_empty()
        && record.risks.is_empty() && record.blockers.is_empty()
    {
...
fn raw_shape_violations(payload: &Value, expected_tid: Option<&str>) -> Vec<CloseoutViolation> {
    let mut violations = Vec::new();
    if payload.get("schema_version").and_then(Value::as_str).map(str::trim).unwrap_or("").is_empty() {
        violations.push(CloseoutViolation::new("schema_version_mismatch", "block",
    ...
    if let Some(expected) = expected_tid.map(str::trim).filter(|s| !s.is_empty()) {
        let actual = payload.get("task_id").and_then(Value::as_str).map(str::trim).unwrap_or("");
        if actual != expected {
    ...
    if let Some(commands) = payload.get("commands_run").and_then(Value::as_array) {
        for (idx, cmd) in commands.iter().enumerate() {
            let ct = cmd.get("command").and_then(Value::as_str).map(str::trim).unwrap_or("");
            let has_ec = cmd.get("exit_code").and_then(Value::as_i64).is_some();
            if ct.is_empty() || !has_ec {
                violations.push(CloseoutViolation::new("invalid_command_evidence", "block",
                    format!("commands_run[{idx}] must include non-empty command and integer exit_code")));
            }
        }
    }
    violations
...
fn blocked_response(payload: Value, violations: Vec<CloseoutViolation>) -> Result<Value> {
    ...
    for v in violations {
        if !resp.violations.iter().any(|e| e.rule == v.rule && e.detail == v.detail) {
...
fn parse_error_response(tid: &str, _violations: Vec<CloseoutViolation>, err: &str) -> Result<Value> {
    let mut resp = base_response(tid);
    resp.violations.push(CloseoutViolation::new("parse_error", "block", format!("parse closeout record failed: {err}")));
    Ok(serde_json::to_value(resp)?)
}
