...
pub fn validate_external_research_strict(v: &Value) -> Result<(), FrameworkError> {
    ...
    let Some(unk) = obj.get("unknowns") else {
        return Err(FrameworkError::validation(
            "external_research strict: missing `unknowns` key (use [] or null)",
        ));
    };
...
pub fn validate_external_research_structured(v: &Value) -> Result<(), FrameworkError> {
    let obj = v
        .as_object()
        .ok_or_else(|| FrameworkError::validation("external_research must be a JSON object"))?;

    let claims = obj
        .get("claims")
        .ok_or_else(|| FrameworkError::validation("external_research missing `claims`"))?;
    let claims = claims
        .as_array()
    ...
    for (i, c) in claims.iter().enumerate() {
        ...
        let sources = row
            .get("sources")
            .ok_or_else(|| FrameworkError::validation(format!("{ctx}: missing `sources`")))?;
        let sources = sources
            .as_array()
            .ok_or_else(|| FrameworkError::validation(format!("{ctx}: sources must be array")))?;
