...
use crate::engine::*;
use crate::helpers::*;
use crate::state::summarize_environment_fingerprint;
use crate::state::summarize_git_provenance;
...
pub(crate) fn sync_managed_file(
    path: &Path,
    fallback: &str,
    start: &str,
    end: &str,
...
fn write_if_missing(path: &Path, content: String) -> anyhow::Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
    if !path.exists() {
...
fn format_string_list_inner(values: &[String], empty: &str) -> String {
    format_string_list(values, empty)
}
...
fn render_findings_summary(state: &Value) -> String {
    ...
    if let Some(entry) = crate::adapters::latest_external_research(state) {
        lines.extend([
            String::new(),
            ...
            format!(
                "- results: {}",
                crate::adapters::external_research_result_count(entry)
            ),
        ]);
    }
...
fn render_current_context_summary(state: &Value) -> String {
    let freshness = state_freshness_public(state);
...
pub(crate) fn sync_workspace_files(workspace: &Path, state: &Value) -> anyhow::Result<()> {
...
// ── status / resume formatting ────────────────────────────────────────

pub(crate) fn format_status(state: &Value) -> String {
    let mut lines = vec![
...
pub(crate) fn format_resume(state: &Value) -> String {
...
fn format_run_record(record: &Value) -> String {
    ...
    [
        ...
        "### Applies To",
        "",
        &format_string_list_inner(&applies_to, "_Where this result can be reused._"),
        "",
        "### Does Not Apply To",
        "",
        &format_string_list_inner(&does_not_apply_to, "_Boundary conditions for reuse._"),
        "",
        "### Reuse Note",
        "",
        &str_field_default(
        ...
        "## Sanity Checks",
        "",
        &format_string_list_inner(&sanity_checks, "_Fill in sanity checks here._"),
        "",
        ...
        "### Rules In",
        "",
        &format_string_list_inner(&rules_in, "_What did this result support?_"),
        "",
        "### Rules Out",
        "",
        &format_string_list_inner(&rules_out, "_What did this result eliminate?_"),
        "",
        "## Alternative Explanations",
        "",
        &format_string_list_inner(
        ...
        "",
        &format_string_list_inner(&threats, "_What could make this conclusion misleading?_"),
        "",
        "## Interpretation",
        "",
        &str_field_default(
...
// ── Freshness helper exposed for render ───────────────────────────────

struct RenderFreshness {
    stale: bool,
...
fn state_freshness_public(state: &Value) -> RenderFreshness {
    ...
    let recent_decisions = {
        ...
        let active_id = state.get("active_hypothesis").and_then(Value::as_str);
        if let Some(active_id) = active_id {
            let recent: Vec<Value> = decisions
                .iter()
                .filter(|d| {
