...
// Re-export constants from runtime-storage
use rt_storage::runtime_envelope_ids::{
    DEFAULT_BACKGROUND_JOB_TIMEOUT_SECONDS, DEFAULT_COMPUTE_THREADS, DEFAULT_MAX_BACKGROUND_JOBS,
...
static DISPATCH_STDIO_JSON_REQUEST: OnceLock<DispatchFn> = OnceLock::new();
static ENV_USIZE: OnceLock<EnvUsizeFn> = OnceLock::new();

/// Register the stdio dispatch function. Called by runtime-core at bootstrap.
pub fn register_stdio_dispatch(dispatch: DispatchFn, env_usize: EnvUsizeFn) {
    let _ = DISPATCH_STDIO_JSON_REQUEST.set(dispatch);
...
fn dispatch_stdio_json_request_payload(request: StdioJsonRequestPayload) -> StdioJsonResponsePayload {
    if let Some(f) = DISPATCH_STDIO_JSON_REQUEST.get() {
        ...
        StdioJsonResponsePayload {
            ...
            ok: false,
            payload: None,
            error: Some("stdio dispatch not registered".to_string()),
        }
    }
}

fn env_usize(key: &str) -> Option<usize> {
    if let Some(f) = ENV_USIZE.get() {
        f(key)
    } else {
...
pub fn handle_stdio_json_line(line: &str) -> StdioJsonResponsePayload {
