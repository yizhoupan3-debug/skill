...
fn main() {
    ...
    // --- host_projection_mcp_relative ---
    {
        let projections = reg
            .get("host_projections")
            .and_then(|v| v.as_object())
            .expect("RUNTIME_REGISTRY missing host_projections");
        out.push_str("pub fn host_projection_mcp_relative(host_id: &str, scope: &str) -> &'static str {\n");
                ...
                .and_then(|v| v.as_array())
            {
                for entry in entries {
                    if let (Some(s), Some(rel)) = (
                        ...
                        out.push_str(&format!("        (\"{host_id}\", \"{s}\") => \"{escaped}\",\n"));
                    }
                }
            }
        }
    ...
    // --- host_projection_mcp_base_is_account ---
    {
        let projections = reg
            .get("host_projections")
            .and_then(|v| v.as_object())
        ...
        out.push_str("pub fn host_projection_mcp_base_is_account(host_id: &str, scope: &str) -> bool {\n");
        out.push_str("    match (host_id, scope) {\n");
        for host_id in &supported {
            if let Some(entries) = projections
                .get(host_id.as_str())
                ...
                .and_then(|v| v.as_array())
            {
                for entry in entries {
                    if let Some(s) = entry.get("scope").and_then(|v| v.as_str()) {
                        let is_account = entry.get("base").and_then(|v| v.as_str()) == Some("account_home");
                        if is_account {
    ...
    // --- ALL_HOST_WORKSPACE_ROOT_ENV_VARS ---
    {
