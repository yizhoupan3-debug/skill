...
use super::types::{
    BackgroundRunStatus, BackgroundStateStore, BACKGROUND_STATE_REQUEST_SCHEMA_VERSION,
    BACKGROUND_STATE_SCHEMA_VERSION, STALE_ACTIVE_HEARTBEAT_TTL_SECS, STALE_TERMINAL_JOB_TTL_SECS,
};
use chrono::Utc;
...
use std::sync::Arc;
    ...
    fn state_persistence_roundtrip_via_memory_backend() {
        ...
        assert_eq!(jobs[0].get("status").and_then(Value::as_str), Some("running"));
        let sessions = state.get("active_sessions").and_then(Value::as_array).unwrap();
