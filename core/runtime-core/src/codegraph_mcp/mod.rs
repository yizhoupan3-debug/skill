...
pub fn run_codegraph_mcp_stdio_loop(repo_root: Option<&Path>) -> Result<(), core_errors::FrameworkError> {
    let repo_root = resolve_repo_root_arg(repo_root)?;
    codegraph_rs::mcp::run_stdio_mcp(&repo_root).map_err(|err| core_errors::FrameworkError::validation(err.to_string()))
}
