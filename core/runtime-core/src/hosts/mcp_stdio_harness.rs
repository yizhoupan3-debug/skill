...
use crate::review_gate_engine::{fork_context_from_values, review_independent_reviewer_evidence};
...
static TASK_VIEW_CACHE: OnceLock<
    Arc<std::sync::Mutex<HashMap<PathBuf, (crate::task_state::ResolvedTaskView, Instant)>>>,
> = OnceLock::new();
...
) -> &'static Arc<std::sync::Mutex<HashMap<PathBuf, (crate::task_state::ResolvedTaskView, Instant)>>> {
...
pub fn run_claude_desktop_mcp_loop(repo_root_arg: Option<&Path>) -> Result<(), String> {
    let _ = resolve_repo_root_arg(repo_root_arg)?;
    eprintln!(
...
fn task_lifecycle_profile(task_view: &crate::task_state::ResolvedTaskView) -> &str {
    task_view
        .goal_state
        .as_ref()
...
fn mcp_closeout_gate_mode_narrative(
    repo_root: &Path,
    host_id: &str,
    host_name: &str,
    ...
    crate::runtime_registry::harness_capability_exception_rationale(
        repo_root,
        host_id,
...
fn mcp_closeout_hard_block_metadata(
    repo_root: &Path,
    host_id: &str,
    lifecycle_profile: &str,
    all_clear: bool,
    ...
    crate::runtime_registry::closeout_evidence_hooks_unsupported_on_host(repo_root, host_id)
}
...
fn desktop_review_evidence_attested(arguments: &Value, repo_root: &Path, task_id: &str) -> bool {
    ...
    let review_lane =
        crate::runtime_registry::is_reviewer_lane_from_registry(lane, Some(repo_root));
...
pub fn evaluate_mcp_closeout_gate(
    ...
    if let Some(rationale) = crate::runtime_registry::harness_capability_exception_rationale(
        repo_root,
        host_id,
        "closeout_evidence_hooks",
    ) {
...
pub fn tool_goal_state_manage_test_helper(
    ...
    let path = crate::mcp_stdio_test_support::unique_temp_repo("goal-manage");
...
pub fn tool_rfv_loop_manage_test_helper(
    ...
    let path = crate::mcp_stdio_test_support::unique_temp_repo("rfv-manage");
    let _ = std::fs::create_dir_all(&path);
...
mod tests {
    ...
    fn unique_test_repo(name: &str) -> PathBuf {
        let path = crate::mcp_stdio_test_support::unique_temp_repo(&format!("mcp-{name}"));
        let _ = std::fs::remove_dir_all(&path);
