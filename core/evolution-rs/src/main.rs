...
mod tests {
    ...
    #[test]
    fn stem_removes_ed_suffix() {
        assert_eq!(stem("tested"), "test");
        assert_eq!(stem("configured"), "configur");
    }

    #[test]
    fn stem_removes_ment_suffix() {
        assert_eq!(stem("management"), "manage");
    }

    #[test]
    fn stem_removes_s_suffix() {
        assert_eq!(stem("tests"), "test");
        assert_eq!(stem("skills"), "skill");
    }

    #[test]
    fn stem_preserves_ss_suffix() {
    ...
    #[test]
    fn entry_is_recent_within_cutoff() {
        let entry = AuditJournalEntry {
            ts: "2026-06-10T12:00:00Z".to_string(),
            ..Default::default()
        };
        let cutoff = DateTime::parse_from_rfc3339("2026-06-09T00:00:00Z")
            .unwrap()
            .with_timezone(&Utc);
    ...
    #[test]
    fn entry_is_recent_outside_cutoff() {
        let entry = AuditJournalEntry {
    ...
    #[test]
    fn entry_is_recent_invalid_ts_defaults_true() {
        let entry = AuditJournalEntry {
            ts: "not-a-date".to_string(),
            ..Default::default()
        };
        let cutoff = DateTime::parse_from_rfc3339("2026-06-09T00:00:00Z")
            .unwrap()
    ...
    fn canonical_skill_name_exact_match() {
        ...
        known.insert("deep-research".to_string());
        assert_eq!(
            canonical_skill_name("deep-research", &known),
            Some("deep-research".to_string())
        );
    }

    #[test]
    fn canonical_skill_name_fallback_to_known() {
        let mut known = HashSet::new();
        known.insert("code-review".to_string());
        assert_eq!(
            canonical_skill_name("code-review + unknown-tool", &known),
            Some("code-review".to_string())
        );
    }

    #[test]
    fn canonical_skill_name_empty_returns_none() {
    ...
    #[test]
    fn canonical_skill_name_slash_separator() {
        let mut known = HashSet::new();
        known.insert("implementx".to_string());
        assert_eq!(
            canonical_skill_name("unknown/implementx/other", &known),
            Some("implementx".to_string())
        );
    }
}
