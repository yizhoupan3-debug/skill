...
#[serde(deny_unknown_fields)]
struct NudgesBody {
    #[serde(alias = "rfv_loop_continue_reasoning_depth", default)]
    qg_loop_continue_reasoning_depth: String,
    #[serde(default)]
    ...
    #[serde(alias = "rfv_loop_external_struct_hint_line", default)]
    qg_loop_external_struct_hint_line: String,
}

#[derive(Debug, Clone)]
pub struct ResolvedHarnessNudges {
    pub qg_loop_continue_reasoning_depth: String,
    ...
    pub qg_loop_external_struct_hint_line: String,
}

impl ResolvedHarnessNudges {
    fn disabled() -> Self {
        Self {
            qg_loop_continue_reasoning_depth: String::new(),
            ...
            qg_loop_external_struct_hint_line: String::new(),
        }
    }
}
...
fn builtin_defaults() -> ResolvedHarnessNudges {
    ResolvedHarnessNudges {
        qg_loop_continue_reasoning_depth: "推理深度：不靠单模型拉长 CoT；靠 review∥external→fix→verify 分工 + EVIDENCE_INDEX 可审计链。"
            .to_string(),
...
pub fn resolve_harness_operator_nudges(repo_root: &Path) -> ResolvedHarnessNudges {
    ...
    merge_nonempty(
        &mut out.qg_loop_continue_reasoning_depth,
        &file.nudges.qg_loop_continue_reasoning_depth,
    );
    merge_nonempty(
    ...
    merge_nonempty(
        &mut out.qg_loop_external_struct_hint_line,
...
mod tests {
    ...
    fn resolve_uses_builtin_when_file_missing() {
        ...
        assert!(
            n.qg_loop_continue_reasoning_depth
    ...
    fn resolve_overrides_from_json() {
        ...
        write!(
            f,
            r#"{{"schema_version":"harness-operator-nudges-v1","nudges":{{"qg_loop_continue_reasoning_depth":"CUSTOM_RFV_NUDGE"}}}}"#
        )
        .unwrap();
        drop(f);
        let n = resolve_harness_operator_nudges(&tmp);
        assert_eq!(n.qg_loop_continue_reasoning_depth, "CUSTOM_RFV_NUDGE");
    ...
    fn schema_version_mismatch_falls_back_to_builtin_defaults() {
        ...
        write!(
            f,
            r#"{{"schema_version":"harness-operator-nudges-v999","nudges":{{"qg_loop_continue_reasoning_depth":"NEW_V999_VALUE_SHOULD_BE_IGNORED"}}}}"#
        )
        .unwrap();
        drop(f);
        let n = resolve_harness_operator_nudges(&tmp);
        ...
        assert!(
            !n.qg_loop_continue_reasoning_depth.contains("V999"),
    ...
    fn operator_inject_kill_switch_disables_nudges() {
        ...
        let n = resolve_harness_operator_nudges(&tmp);
        assert!(
            n.qg_loop_continue_reasoning_depth.is_empty(),
    ...
    fn env_off_yields_empty() {
        ...
        std::fs::create_dir_all(&tmp).unwrap();
        let n = resolve_harness_operator_nudges(&tmp);
        assert!(n.qg_loop_continue_reasoning_depth.is_empty());
        assert!(n.retrieval_trace_harness_line.is_empty());
        assert!(n.qg_loop_external_struct_hint_line.is_empty());
        match prior {
            Some(v) => unsafe { core_state_utils::env_sync::set_env(HARNESS_NUDGES_ENV, &v) },
