...
fn runtime_fields_payload() -> Map<String, Value> {
    match json!({
        "shared": EXECUTION_SHARED_RUNTIME_FIELDS,
        "live_primary_required": EXECUTION_LIVE_PRIMARY_REQUIRED_RUNTIME_FIELDS,
...
pub fn build_steady_state_execution_kernel_metadata(response_shape: &str) -> Map<String, Value> {
    match json!({
        "execution_kernel_metadata_schema_version": EXECUTION_METADATA_SCHEMA_VERSION,
        "execution_kernel": EXECUTION_KERNEL_KIND,
        "execution_kernel_authority": EXECUTION_KERNEL_AUTHORITY,
        "execution_kernel_contract_mode": EXECUTION_KERNEL_CONTRACT_MODE,
        "execution_kernel_fallback_policy": EXECUTION_KERNEL_FALLBACK_POLICY,
        "execution_kernel_in_process_replacement_complete": true,
        "execution_kernel_delegate": EXECUTION_KERNEL_DELEGATE_IMPL,
        "execution_kernel_delegate_authority": EXECUTION_AUTHORITY,
        ...
        "execution_kernel_live_primary": EXECUTION_KERNEL_DELEGATE_IMPL,
...
pub fn build_pre_tool_use_guard_execution_contract() -> Value {
    let mut guard = crate::pre_tool_use_guard::pre_tool_use_guard_contract();
...
fn normalize_execution_kernel_metadata_contract_impl(
    ...
    if payload_fields != expected_fields {
        return Err(FrameworkError::validation(
            ERR_UNEXPECTED_STEADY_STATE_FIELDS,
        ));
    }
