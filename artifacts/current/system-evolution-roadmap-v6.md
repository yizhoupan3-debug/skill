...
**Phase 1（低风险，高收益）**：
1. ~~删除 `ensure_research_mcp_five_host_surfaces()` 及两个子函数（项目 `.mcp.json` 写入）~~ ✅ DONE (2026-06-11)：从 host-projection/projection.rs 删除（零调用者）
...
### 1.7 退役环境变量清除 + 重命名 [2d] — I12 ✅ DONE (2026-06-11)

**已执行**：
- **删除 9 个死函数**（runtime-core/router_env_flags.rs）：`router_rs_cursor_autopilot_pre_goal_enabled`、`router_rs_cursor_hook_state_legacy_full_sweep_enabled`、`router_rs_cursor_pre_goal_strict_disk_enabled`、`router_rs_cursor_hook_legacy_subtracted_events_enabled`、`router_rs_cursor_hook_state_fail_open_enabled`、`router_rs_cursor_cargo_check_sync_enabled`、`router_rs_cursor_hook_state_dir_sync_enabled`、`router_rs_cursor_hook_state_stale_sweep_days`、`router_rs_cursor_hook_silent_enabled` + 2 个关联测试
- **重命名 5 个**：`ROUTER_RS_CURSOR_*` → `ROUTER_RS_*` canonical（保留旧名 fallback）
- host-projection 的 hooks.rs 中同名 proxy 层为活代码，保留
- 1685+ tests passed
...
| 指标 | 数据 |
|------|------|
| workspace 编译 | **0 error**, 141 warnings |
...
~~**编译性能瓶颈**：`screenshot_rs` 依赖 xcap（macOS 原生框架绑定），编译 13.5s；`image_process_rs` 依赖 image 0.24.9，编译 5.3s。清理后（§3.4）预计节省 ~20s。~~ ✅ §3.4 已清理，screenshot_rs/image_process_rs 已从 workspace 移除。

### B. 测试

| 指标 | 数据 |
|------|------|
| 全量测试 | **1,685+ passed, 5 failed (预存), ~40 ignored** |
| 测试执行时间 | ~30s（host-projection 最慢 26s，其余 <8s） |

**🔴 5 个预存失败测试**（未修改代码上同样失败，非 v6 改动引起）：

| 测试 | crate | 原因 |
|------|-------|------|
| `codex_sync_preserves_existing_agents_codex_delta_file` | host-integration | `host 'cursor' does not yet have an entrypoint provider` |
| `codex_sync_does_not_write_root_agents_md` | host-integration | 同上 |
| `install_skills_uses_rust_only_entrypoints` | host-integration | 同上 |
| `paper_prose_quality_hook_txt_exists_and_nl_signal_registered` | policy-contracts | 未验证 |
| `project_host_skill_projection_is_generated_outside_host_entrypoints` | host-integration | 同上 |
...
| `*/host_integration/projection.rs` | 4,044 (仅 host-projection) | ✅ 已修复：runtime-core 副本已删除，改为 re-export |
...
- ~~**`projection.rs` 重复**~~ ✅ 已修复 (2026-06-11)：runtime-core 副本（4047 行）已删除，改为 `pub use host_projection::host_integration::projection` re-export
...
| 包 | 版本冲突 | 来源 |
|----|---------|------|
| `bitflags` | ~~1.3.2 vs 2.11.1~~ ✅ 已消除 | §3.4 清理 image_process_rs 后自动解决 |

~~**根因**：`image_process_rs` 依赖的 `image = "0.24"` 已过时（最新 0.25.x 使用 bitflags v2）。清理 screenshot_rs/image_process_rs（§3.4）后自动消除。~~ ✅ 已消除。
...
| rust_tools crate | 6 | §3.4 清理后（原 11 → 删除 5 个孤立 crate） |
| integration tests | 2 | 根 crate + common |
