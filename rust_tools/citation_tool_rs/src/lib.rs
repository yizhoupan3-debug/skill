//! Core types and logic for citation audit, lint, and reference rendering.
...
impl BibEntry {
    ...
    pub fn first_author_norm(&self) -> String {
...
pub struct AuditReport {
    ...
    pub likely_preprints: Vec<String>,
    pub missing_doi: Vec<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub manuscript_consistency: Option<ManuscriptConsistency>,
}
...
pub struct AuditSummary {
    ...
    pub entries_with_missing_required_fields: usize,
    pub likely_preprints: usize,
    pub article_or_conference_entries_missing_doi: usize,
    pub blocking_issue_count: usize,
    pub warning_issue_count: usize,
}

#[derive(Serialize)]
pub struct ManuscriptConsistency {
    pub total_in_text_citation_mentions: usize,
    pub unique_cited_keys: usize,
    pub missing_in_bibliography: Vec<String>,
    pub uncited_reference_entries: Vec<String>,
    pub repeated_citation_keys: BTreeMap<String, usize>,
    pub dense_citation_sentences: Vec<DenseCitationSentence>,
}

#[derive(Serialize, Clone)]
pub struct DenseCitationSentence {
...
// ---------------------------------------------------------------------------
// Sentence / citation cluster analysis
// ---------------------------------------------------------------------------
...
/// Build a full audit report for the given BibTeX entries.
pub fn make_report(
    entries: &[BibEntry],
...
/// Check audit report against a fail-on policy.
pub fn enforce_audit_fail_on(report: &AuditReport, fail_on: FailOn) -> Result<()> {
    match fail_on {
        FailOn::Never => Ok(()),
...
pub fn audit_report_to_markdown(report: &AuditReport) -> String {
    let mut lines = vec![
...
/// Lint manuscript text for dense / sentence-ending citation clusters.
pub fn lint_claims(text: &str, threshold: usize) -> Result<Vec<ClaimFinding>> {
...
/// Render claim-lint findings as Markdown.
pub fn claim_findings_to_markdown(findings: &[ClaimFinding]) -> String {
...
/// Render a single bibliography entry in the given citation style.
pub fn render_entry(entry: &BibEntry, style: ReferenceStyle) -> String {
...
"#
    ...
    fn file_backed_audit_and_render_work_without_python() {
        ...
        std::fs::write(&bib, fixture_bib()).expect("write bib");
        std::fs::write(
            &manuscript,
