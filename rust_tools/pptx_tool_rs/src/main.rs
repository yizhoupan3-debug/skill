...
fn deck_template_from_outline(outline: &Value) -> DeckTemplate {
    let raw = outline
        .get("template")
...
fn notes_with_semantic_layout(layout: &'static str, continuation: &str) -> String {
    format!(
...
fn slide_background_and_rail(palette: &PptPalette) -> Vec<String> {
    vec![
        rect_shape(
            2,
            "Background",
...
fn slide_pack_xml(title_escaped: &str, shapes_html: &str) -> String {
    format!(
...
fn slide_shapes_label(slide: &PptxSlideSpec, palette: &PptPalette) -> String {
    text_shape(
        10,
        "Slide Label",
        &slide.label,
...
fn slide_shapes_cover_or_closing(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
...
fn slide_shapes_standard_content(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
    let panel = rect_shape(
        4,
        "Content Panel",
        ShapeBox {
...
fn slide_shapes_comparison(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
...
fn slide_shapes_hero_image(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
...
fn slide_shapes_image_text_split(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
...
fn slide_shapes_data_panel(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
    let panel = rect_shape(
        4,
        "Content Panel",
        ShapeBox {
...
fn slide_shapes_step_lane(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
    let panel = rect_shape(
        4,
        "Content Panel",
        ShapeBox {
...
fn slide_shapes_multi_card(slide: &PptxSlideSpec, palette: &PptPalette) -> Vec<String> {
    let placements = [
...
fn slide_shapes_page_footer(slide_no: usize, total: usize, palette: &PptPalette) -> String {
    text_shape(
        40,
        "Page",
...
fn count_ai_slop_hits_in_element(element: &Value) -> usize {
    let mut hits = 0usize;
    let text = element
...
fn count_ai_slop_hits(text: &str) -> usize {
...
fn vague_promise_hits(text: &str) -> usize {
...
fn has_any_evidence_token(text: &str) -> bool {
...
#[derive(Debug, Clone, Copy)]
struct SlideTextStats {
    text_box_count: usize,
    text_char_count: usize,
}

fn slide_text_stats(elements: &[Value]) -> SlideTextStats {
...
fn collect_text_stats(element: &Value, stats: &mut SlideTextStats) {
    let full_text = element
        .pointer("/text/fullText")
        .and_then(Value::as_str)
        .map(str::trim)
...
fn slide_has_rust_semantic_marker(slide: &Value) -> bool {
    slide
        .get("notes")
        .and_then(Value::as_str)
...
fn semantic_layout_role_from_slide(slide: &Value) -> String {
    slide
        .get("notes")
        .and_then(Value::as_str)
        .and_then(|notes| {
...
fn deck_notes_have_semantic_markers(slides: &[Value]) -> bool {
...
fn find_title_candidate(elements: &[Value], slide_h: f64) -> Option<(f64, f64, f64, f64)> {
...
fn element_rect(element: &Value) -> Option<(f64, f64, f64, f64)> {
    let position = element.get("position")?;
