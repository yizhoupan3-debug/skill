//! Core logic for gh-source-gate — PR checks inspection, review comments, and doctor diagnostics.
...
pub const DEFAULT_MAX_LINES: usize = 160;
pub const DEFAULT_CONTEXT_LINES: usize = 30;
...
pub const REVIEW_THREADS_QUERY: &str = r#"query(
  $owner: String!,
  $repo: String!,
  $number: Int!,
...
// ---------------------------------------------------------------------------
// Public API — returns JSON Value for both CLI and MCP callers
// ---------------------------------------------------------------------------

/// Inspect failing GitHub Actions checks for a PR. Returns a JSON Value with
/// `pr` (number) and `results` (array of analyzed check objects).
pub fn inspect_pr_checks_json(
    repo_root: &Path,
    pr: Option<&str>,
    max_lines: usize,
    context: usize,
    ...
    let pr_number = resolve_pr(pr, repo_root)?;
    let checks = fetch_checks(&pr_number, repo_root)?;
    ...
    if failing.is_empty() {
        return Ok(json!({
            ...
            "message": format!("PR #{}: no failing checks detected.", pr_number),
        }));
    }

    let results: Vec<Value> = failing
        .into_iter()
        .map(|check| analyze_check(check, repo_root, max_lines.max(1), context.max(1)))
        .collect::<Result<_>>()?;

    Ok(json!({
        "pr": pr_number,
        "failing_check_count": results.len(),
        "results": results,
    }))
}
...
/// Run doctor diagnostics on the repository. Returns a JSON Value with
/// `status` ("ok" or "failed") and `checks` (array of check objects).
pub fn doctor_json(repo_root: &Path) -> Result<Value> {
    let report = build_doctor_report(repo_root);
...
pub fn build_doctor_report(start: &Path) -> DoctorReport {
...
fn check_cli_source_owns_commands(repo_root: &Path) -> DoctorCheck {
    // Read both main.rs and lib.rs — the crate is now split
    let main_source =
...
pub fn one_line(value: &str, max_len: usize) -> String {
...
// ---------------------------------------------------------------------------
// Rendering helpers (CLI-only, kept here for completeness)
// ---------------------------------------------------------------------------

pub fn render_check_results(pr: &str, results: &[Value]) {
...
pub fn render_comment_summary(payload: &Value) {
...
"#,
        ...
        write_file(
            &root.join("rust_tools/gh_source_gate_rs/src/lib.rs"),
            r#"
