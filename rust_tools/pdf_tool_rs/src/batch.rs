    ...
    fn resolve_jobs_auto_caps_at_eight() {
        with_pdf_batch_jobs_env(|| {
            let cpus = std::thread::available_parallelism()
                .map(|n| n.get())
                .unwrap_or(4);
            assert_eq!(resolve_jobs(JOBS_AUTO, &[]), std::cmp::min(8, cpus).max(1));
        });
    }
    #[test]
