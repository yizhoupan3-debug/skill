...
- **MCP transport**（无 shell hook）：`opencode.json` → `mcpServers.router-rs-framework`；项目 `.opencode/opencode.json`、用户 `~/.config/opencode/opencode.json`。
...
- **联网**：无原生外网工具；可经 MCP 注册 `browser-mcp`。
