Create a single horizontal sprite strip for the Codex app digital pet `rainbow-duck-jiji` in the state `review`.
...
Motion readability repair: make the review loop visibly animated at 192x208. Across the 6 frames, show a clear lean forward, head tilt, blink/focused eye change, bill dip, one wing lifting as if concentrating, and nightcap pom bounce. Do not add magnifying glasses, papers, code, UI, punctuation, or detached symbols.

Rock Kingdom mobile action redesign: make review feel like a curious sleepy duck inspecting something just below its bill. Use a clear forward lean, a side-to-side head tilt, bill dipping, one wing raised to balance the round body, slippers gripping in place, and a delayed nightcap swing. The motion should be character acting, not props.

Final work-state redesign: turn this state into "coding mode." The duck sits cross-legged or slipper-crossed in the same frame slot, with a tiny pixel laptop or keyboard directly on its lap/touching its belly. Both wings type on the keyboard in alternating poses. The screen may show tiny abstract pixel blocks only; no readable text, code strings, UI labels, symbols, or floating effects. Preserve the nightcap, starry purple body, orange bill, sleepy eyes, and slippers.

Final polish directive, superseding any earlier exaggeration: keep the seated duck and laptop at a stable size in all 6 frames. Make the motion smooth through wing taps, blink, slight bill bob, tiny shoulder bounce, and nightcap pom sway. The body should stay seated with almost no vertical pop. Frame 5 must loop naturally back to frame 0.
