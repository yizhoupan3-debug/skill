Create a single horizontal sprite strip for the Codex app digital pet `rainbow-duck-jiji` in the state `waiting`.
...
Motion readability repair: make the waiting loop visibly animated at 192x208 while still patient. Across the 6 frames, show a clear weight shift from one slipper to the other, sleepy blink, small body sway, bill bob, wing repositioning, and nightcap pom bounce. Use distinct poses, not near-identical copies.

Rock Kingdom mobile action redesign: make waiting feel like impatient cute Duck Jiji energy. It should rock from slipper to slipper, do one quick "about to dash" lean, pull itself back, blink sleepily, and let the nightcap pom swing in a broad arc. Keep it contained, but make the weight shift readable in silhouette.

Final polish directive, superseding any earlier exaggeration: keep scale and footprint consistent. The waiting loop should be a smooth side-to-side weight shift with small lean, blink, and hat-pom sway. Do not introduce a sudden crouch, stretch, or size pop. Frame 5 must ease back into frame 0.
