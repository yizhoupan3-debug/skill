Create a single horizontal sprite strip for the Codex app digital pet `rainbow-duck-jiji` in the state `running-right`.
...
Motion readability repair: make the run clearly readable at 192x208. Across the 8 frames, show an exaggerated rightward run-in-place cycle: alternating feet far forward/back, slippers visibly swinging, body leaning forward then rebounding, small airborne frames, wings counter-swinging, bill and head bobbing, and the nightcap pom bouncing. Use distinct cycle phases, not near-identical standing poses. Keep every pose complete and inside its slot.

Rock Kingdom mobile action redesign: make this feel like an urgent "pa-da pa-da" Duck Jiji dash from the mobile game. Use a waddling sprint, not a generic jog: the round body leans strongly to the right, one slipper kicks high forward while the other pushes far back, wings flap out for balance, the bill points slightly into the run, and the nightcap trails behind then snaps forward. Include two clear airborne squash/stretch beats, but no speed lines, dust, shadows, or detached effects.

Final polish directive, superseding any earlier exaggeration: make the gait lively but size-stable. The duck should not shrink, grow, or change body mass between frames. Keep body height and width within about 10% across the row, keep the bill size constant, and make the hat pom lag smoothly without jumping. Show motion through leg, slipper, wing, lean, and cap positions, not through scaling the whole pet. Frame 7 must loop smoothly back to frame 0.
