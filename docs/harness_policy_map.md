...
| Cursor Stop 门控编排 | [`ship_readiness.rs`](../core/runtime-core/src/ship_readiness.rs) + [`handlers_stop.inc.rs`](../core/host-projection/src/hosts/cursor_hooks/handlers_parts/handlers_stop.inc.rs) | 每轮 Stop **最多一条** hard `followup_message`：closeout → REVIEW_GATE → AG_FOLLOWUP（`primary_fix=`）；`my-light` 仅 closeout + 软提示 |
