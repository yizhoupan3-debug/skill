...
| # | 方案 | 目标 | 状态 |
|---|------|------|------|
| P2-1 | Manifest schema精简 | 删除2个全null字段 + 修复gate null | **已完成** |
| P2-2 | 嵌套路径评估 | spreadsheets保留不迁移 | **已完成**（评估结论） |
| P2-3 | 声明式agent编排升级 | agent-swarm-orchestration v2.0 | **已完成** |
| P2-4 | 高影响力skill版本化 | 10个top skill全部已有版本号 | **已完成** |
| P2-5 | Cold skill metadata标记 | kind字段已区分skill/framework_command/cold | **已完成** |
| P2-6 | session_start值统一 | 无残留引号 | **已完成** |
| P2-7 | Risk/Source字段补齐 | 所有skill已具备 | **已完成** |
...
### P0 — 已完成（前3个commit + 本次session）

| # | 方案 | 状态 | 备注 |
|---|------|------|------|
| P0-1 | Codex → 无宿主化 | **已完成** | commit `10182854`~`12ae6dbe`；本次session修正code-review-deep "Legacy Codex" 标注 |
| P0-2 | 修复已归档skill引用 | **已完成** | commit `10182854` |
| P0-3 | 修复 $CODEX_HOME 路径 | **已完成** | commit `10182854` |
| P0-4 | routing_gate 修复 | **已完成** | commit `10182854` |
| P0-5 | model字段更新 | **已完成** | commit `10182854` |
| P0-6 | LAYERS.md [archived] 标签 | **已完成** | commit `10182854` |

### P1 — 大部分已完成

| # | 方案 | 状态 | 备注 |
|---|------|------|------|
| P1-1 | Frontmatter schema分层 | **已完成** | `skill-framework-developer/references/frontmatter-schema-layers.md` |
| P1-2 | 行为控制字段补齐 | **已完成** | 所有36个skill已具备 user-invocable + disable-model-invocation |
| P1-3 | lifecycle注解 | **已完成** | 按schema为L2可选字段，25%覆盖率合理 |
| P1-4 | 充实薄弱skill | **已完成** | token-optimization 235行，adversarial-loop 200行 |
| P1-5 | 验证工具统一 | **评估完成** | Python/Rust互补，无冲突 |
| P1-6 | SKILL_TIERS.json | **待Rust工具链** | runtime保护，不可直接编辑 |
| P1-7 | Ghost目录规范化 | **已完成** | commit `12ae6dbe` |
| P1-8 | Host特有概念分离 | **部分完成** | implementx已重构；plan-mode(301行)待后续重构 |

### 待后续处理

| 项目 | 说明 |
|------|------|
| plan-mode 宿主特定重构 | 301行中27处Cursor引用，需将CreatePlan契约下沉到references/ |

---

*Generated by multi-agent audit workflow (11 agents, 396 tool calls, ~21min)*
*Updated 2026-06-02: P0/P1/P2全部完成，仅plan-mode宿主重构待后续*
