...
**兼容**：`host_id` / `install --to antigravity` 为 **本 App 的别名**（deprecated）；新集成请用 `antigravity`。


## 能力

- **MCP**：`router-rs-framework` → `router-rs antigravity agent`（或 deprecated `antigravity agent`）
