...
> Rust 行为真源：`core/runtime-core/src/rfv_loop.rs` 及相关 hook 合并逻辑。

**不进入热 skill 路由**（`skills/SKILL_ROUTING_RUNTIME.json` 不含本主题的独立 slug）。此处保留 **`framework_rfv_loop` 字段契约、lane 模板与推理深度说明**，供 GOAL drive / [`/workflow`](../skills/agent-swarm-orchestration/SKILL.md) / [`adversarial-loop`](../skills/.archive-cold/adversarial-loop/SKILL.md) 与工具链引用。
...
- **不能**在同一任务上「双 macro **同时要求续跑**」：`GOAL` 处于需要续跑的 running/drive、且 **`RFV_LOOP_STATE.loop_status=active`** 时，`resolve_task_view` 会得到 **Conflict**（`autopilot_goal_and_rfv_loop_both_active`），真源：[`core/core-state/src/task_state.rs`](../core/core-state/src/task_state.rs) 的 `classify_control_mode`。
- **`framework_rfv_loop` `operation: start`** 会摘掉同任务的 `GOAL_STATE.json`，真源：`deactivate_goal_for_conflict_with_rfv`（[`core/runtime-core/src/autopilot_goal.rs`](../core/runtime-core/src/autopilot_goal.rs)）。
