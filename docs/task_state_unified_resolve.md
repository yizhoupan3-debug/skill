...
6. **GOAL 写路径**：`GOAL_STATE.json` 的 mutating 真源为 **`framework_goal_drive` stdio**（经 `task_write_lock` + `TASK_LEDGER.jsonl`）；My 四命令 skill **禁止**代理直接 `Write` 整文件。`framework_goal_drive` 的 **`start` / `resume`** 同时维护 `active_task.json`（默认还有 `focus_task.json`）；**`complete` / `clear`** 删除指向该任务的指针。Hook/Stop 只读盘上 GOAL + `EVIDENCE_INDEX`（依赖指针解析 `task_id`）。
...
- 命名写分发（2.5）：`core/runtime-core/src/task_command.rs`
...
维护：若修改 `task_id` 解析或 `control_mode` 分类规则，**同时**更新本文 §2–§3、`core/core-state/src/task_state.rs` 与 [`runtime_view.rs`](../core/runtime-core/src/framework_runtime/runtime_view.rs) 中单测。`framework snapshot` 使用的 `active_task_id` 与 `resolve_task_view` 已对齐（`override > active > focus`）。
