...
- **Multiset 与双事件**：`review_subagent_pending_cycle_keys` 由 qualifying **`subagentStart`** / **`PostToolUse`** 入队；**核销**由匹配的 **`subagentStop`** 或同 id/lane 的 **`PostToolUse` 返回**（Task 已结束）完成，pending 排空后升 phase 3。同一 **`id:`** 若 `subagentStart` 已入队，随后同 id 的 `PostToolUse` **不重复入队**（见 `push_review_pending_cycle_key`）。并行仅 `lane:` 且无稳定 id 时仍依赖 multiset 中多条相同 key。  
