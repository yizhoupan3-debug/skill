...
**Host-neutral entrypoint sync**：`router-rs framework sync-entrypoints --repo-root <repo>` 调用 `sync_host_entrypoints`（支持 `--host-id` 选择 provider；当前实现仅 codex）。
