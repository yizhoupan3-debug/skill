...
- Cursor `review_gate` / Codex **`codex hook`** / **`router-rs claude hook`** 出站 JSON 可含顶层 **`router_rs_observation`**（`core/router-rs/src/router_rs_observation.rs`；hook 宿主 id：`cursor` / `codex` / `claude-code`）。MCP 宿主 JSON-RPC **不** attach `router_rs_observation`。
