...
| 宿主投影不变量 | [host-projection.md](host-projection.md) |
| 路由契约 + 插件 ABI | [routing-and-plugin.md](routing-and-plugin.md) |
| 状态账本 + 可移植性 + 外部基准 | [status-and-portability.md](status-and-portability.md) |
...
- Cursor `review_gate` / Codex **`codex hook`** / **`router-rs claude hook`** 出站 JSON 可含顶层 **`router_rs_observation`**（`core/router-rs/src/router_rs_observation.rs`；hook 宿主 id：`cursor` / `codex` / `claude-code` / `antigravity-cli`）。**`claude-desktop agent`** MCP JSON-RPC **不** attach `router_rs_observation`。
