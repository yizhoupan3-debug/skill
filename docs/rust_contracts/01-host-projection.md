...
- `configs/framework/RUNTIME_REGISTRY.json` is read at runtime for `review_gate` and related registry-backed hook policy through `runtime_registry` (`core/runtime-core/src/runtime_registry/mod.rs`; `` is a re-export shim); steady-state must not rely on compile-time `include_str!` of the registry for those hot paths.
