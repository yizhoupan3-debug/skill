...
- `configs/framework/RUNTIME_REGISTRY.json` is read at runtime for `review_gate` and related registry-backed hook policy through `runtime_registry` (`core/framework-kernel/src/runtime_registry.rs`; `` is a re-export shim); steady-state must not rely on compile-time `include_str!` of the registry for those hot paths.
