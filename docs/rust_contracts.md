...
- Cursor `review_gate` / Codex **`codex hook`** / **`router-rs claude hook`** / **`router-rs claude-desktop agent`** 出站 JSON 可含顶层 **`router_rs_observation`**（`scripts/router-rs/src/router_rs_observation.rs`；宿主 id 与闭集一致：`cursor` / `codex` / `claude-code` / `claude-desktop`）；可选 `correlation.session_id` / `correlation.task_id`。
