...
浏览器自动化 MCP：`browser-mcp` stdio 服务、`session_launch`、页面 attach、与 **web_fetch_guard** 协同的 URL 策略。实现：`core/router-rs/src/browser_mcp/`；Node 侧包由 `host-integration install` 投影注册。
...
- `core/router-rs/src/browser_mcp/`
