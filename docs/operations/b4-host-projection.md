...
将框架统一协议**投影**到各宿主：hook 绑定、MCP 注册、`host_projection_narrative` 文案、生成物 drift 探测。实现：`core/router-rs/src/host_integration/`、各 `hosts/*_hooks`。
...
- `core/router-rs/src/host_integration/`
