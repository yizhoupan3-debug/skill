...
**Codex CLI：`CODEX_REVIEW_GATE`（wave-2 部分，2026-05）**：[`CodexLifecycleContextState`](../scripts/router-rs/src/codex_hooks.rs) 使用 `phase` + `subagent_start_count` + 布尔 `independent_review_subagent_seen`（**无** Cursor multiset）。PostTool 深度 lane 可数证据 → `phase≥2`；Stop 上 compact findings **仅在有可数证据时** 升 `phase=3`（`maybe_bump_codex_review_phase_for_compact_findings`）；`rg_clear` / bounded reject token（`saw_reject_reason`）亦可清门。**compact alone 不得清门**。`my-light` 与 **`ROUTER_RS_CODEX_REVIEW_GATE_DISABLE=1`** 关闭硬拦（对称 Cursor/Claude disable env）。**不**移植 Cursor subagentStart/Stop multiset、phase soft-nag cap、`afterAgentResponse` hook。
