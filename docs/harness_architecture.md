...
| `ROUTER_RS_CURSOR_PRE_GOAL_STRICT_DISK` | 关 | **仅** `1`/`true`/`yes`/`on`：**禁止**仅凭磁盘 `GOAL_STATE` hydration 将 `pre_goal_review_satisfied` 置真（beforeSubmit 与 Stop 均适用）；pre-goal 仍可由 subagent / `reject_reason` / nag cap 等满足 |
