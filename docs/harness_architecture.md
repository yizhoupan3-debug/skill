...
- **`afterAgentResponse` / hook-state**：handler [`handle_after_agent_response`](../scripts/router-rs/src/cursor_hooks/handlers.rs) 仍存在，但本仓 [`.cursor/hooks.json`](../.cursor/hooks.json) **默认不注册**该事件（减法：改依赖 `Stop` tail 清门）。应急分支若宿主仍派发该事件，行为与常态同。PostTool/subagent/start/stop/sessionEnd 等仍走各自 `handle_*`。仅 **beforeSubmit** 两事件在应急下跳过 `handle_before_submit`。
