...
使用者一页速查（误报 `RG_FOLLOWUP` 行、**Stop 上 `REVIEW_GATE` 与 `AG_FOLLOWUP` 的先后**、同轮混写 GSD goal drive 入口时的武装结果；**`/autopilot` 已退役**）：[`framework_operator_primer.md`](framework_operator_primer.md)（§「机读短码真源与常见误报」「混用时的实际武装顺序」）。
...
**Claude Code**：在满足显式 `fork_context=false` 的前提下，[`claude_hooks.rs`](../scripts/router-rs/src/claude_hooks.rs) `reviewer_lane` 读取 `review_gate.claude_reviewer_lanes`（`is_claude_reviewer_lane_from_registry`）：**`deep_gate_lanes` 四拼写加上** `review` / `reviewer` / `critic` / `code-review`；**`explore` / `explorer` 不计入**。**不要将 Claude 专用 lane 套用到 Cursor/Codex**（后两者仅 `deep_gate_lanes`）。跨宿主差异表见 [`host_adapter_contract.md`](host_adapter_contract.md) **§0.1**。
...
| `ROUTER_RS_CONTINUITY_STOP_CHECKPOINT` | 开 | **仅** `0`/`false`/`off`/`no`：关闭 Cursor/Codex `Stop` 对**当前任务**的原地 checkpoint（[`cursor_hooks/handlers.rs`](../scripts/router-rs/src/cursor_hooks/handlers.rs)、[`codex_hooks.rs`](../scripts/router-rs/src/codex_hooks.rs)）；unset 等同启用；**不**再 mint 新 `cursor-stop-*` 或改写 focus/active |
