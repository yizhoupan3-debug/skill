...
- `rust_execute_fallback_to_python` 仅保留为 retired explicit-request surface：
  旧调用方可以显式探测，但 steady-state runtime 只返回 rejection，不再恢复
  Python live path；当前 contract 同时把它标记为 `pending-removal`，删除前提是
  先拿到外部 caller 已停止 probing 的证据，再在同一变更里一起删除设置字段、
  env var、rejection shim 与 artifact 暴露，并重跑定向回归
