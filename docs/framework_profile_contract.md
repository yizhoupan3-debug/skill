...
当前主线口径统一为 `Rust-owned contract truth + thin Python projection`：
...
都由 `router-rs` 编译。Python 只能序列化 profile、读取 Rust 产物、做 schema
校验；不能再维护第二套默认值、bridge 表、fallback emitter 或 Python/Rust
parity lane。
...
- `emit_framework_contract_artifacts(...)` 默认也走 Rust；即使调用方没有显式传
  `rust_adapter`，也会使用 repo-local `RustRouteAdapter`。
- Python `FrameworkProfile.shared_contract_surface()`、memory/mcp/session
  normalization、workspace bootstrap projection 只读取 Rust 编译结果。
- Python artifact emission 不再写 fallback host artifacts，不再写
...
当前默认 `emit_framework_contract_artifacts(...)` 只发布 Rust 编译出的
...
不再由 Python artifact emitter 写出。
...
- `controller_boundary.host_entrypoints` 的默认 peer set 固定为
  ...
  host artifacts 不再由 Python emitter 产出
