...
相关控制面 JSON（非 profile 字段，但 install/门禁消费）：`configs/framework/RUNTIME_REGISTRY.json`（含 `review_gate`）、`configs/framework/host_projection_narrative.json`（GSD/review 安装文案）、`configs/framework/GENERATED_ARTIFACTS.json`（生成物 drift manifest）。叙述见 [`harness_architecture.md`](harness_architecture.md) §2.3。
