...
- **Codex**：积极鼓励多代理并行（详见 [`docs/hosts/codex-cli.md` § 多代理编排](../hosts/codex-cli.md)）。`/implementx` 执行区 `parallel` 模式应主动 spawn 子代理 lane；深度 review 默认 spawn-first（`fork_context=false`）。无 hook 级子代理事件，行为由文档契约约束。
