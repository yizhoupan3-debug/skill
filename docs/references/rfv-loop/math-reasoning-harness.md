...
**编排入口**：多轮 RFV 见 [lane-templates.md](lane-templates.md) 中的数理专项 lane；My 执行区（`/implementx` 等）须在 **Goal 契约**里写明 **双轨/脚本级** `validation_commands`。[`rfv_loop_harness.md`](../../rfv_loop_harness.md) 中的 `verify_commands` 与 **`EVIDENCE_INDEX`** 规则同样适用。
