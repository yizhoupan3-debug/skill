...
use closeout_enforcement::{closeout_enforcement_contract, evaluate_closeout_record_value};
use codex_hooks::{
...
use eval_route::{eval_route_contract, run_eval_route};
...
#[derive(Args, Debug, Clone)]
struct CloseoutEvaluateCommand {
...
fn dispatch_cursor_command(command: CursorCommand) -> Result<(), String> {
    match command {
        CursorCommand::Hook(command) => {
            run_cursor_review_gate(&command.event, command.repo_root.as_deref())
        }
    }
}
...
fn dispatch_closeout_command(command: CloseoutCommand) -> Result<(), String> {
    match command {
        CloseoutCommand::Evaluate(args) => {
            ...
            let response = evaluate_closeout_record_value(record_value)?;
...
fn classify_stdio_op(op: &str) -> Option<StdioOpDomain> {
...
#[cfg(test)]
fn is_routing_stdio_op(op: &str) -> bool {
    op_in_domain(op, ROUTING_STDIO_OPS)
}

#[cfg(test)]
fn is_runtime_stdio_op(op: &str) -> bool {
    op_in_domain(op, RUNTIME_STDIO_OPS)
}

#[cfg(test)]
fn is_trace_stdio_op(op: &str) -> bool {
    op_in_domain(op, TRACE_STDIO_OPS)
}

#[cfg(test)]
fn is_framework_stdio_op(op: &str) -> bool {
...
fn dispatch_routing_stdio_request(op: &str, payload: Value) -> Result<Value, String> {
    match op {
        ...
        "closeout_evaluate" => evaluate_closeout_record_value(payload),
...
const EXECUTE_AGGREGATOR_HOST_ALLOWLIST_ENV: &str = "ROUTER_RS_EXECUTE_AGGREGATOR_HOST_ALLOWLIST";
...
fn infer_research_mode(payload: &ExecuteRequestPayload) -> ResearchMode {
    ...
    if task.contains("/autopilot-deep")
        || task.contains("/autopilot deep")
        || task.contains("深度调研")
        || task.contains("深度研究")
    {
    ...
    for reason in &payload.reasons {
        ...
        if lowered.contains("深度调研") || lowered.contains("deep research") {
            return ResearchMode::Deep;
        }
    }
...
const DEEP_CONTINUATION_ASSISTANT_TAIL_CHARS: usize = 1600;
...
fn perform_live_execute_with_sender<F>(
    payload: &ExecuteRequestPayload,
    prompt_preview: &str,
    mut send_request: F,
...
fn validate_live_execute_aggregator_base_url(base_url: &str) -> Result<(), String> {
