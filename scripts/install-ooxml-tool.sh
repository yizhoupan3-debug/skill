...
MANIFEST="${FRAMEWORK_ROOT}/rust_tools/ooxml_parser_rs/Cargo.toml"
BIN_DIR="${OOXML_BIN_DIR:-${HOME}/.local/bin}"
DEST="${BIN_DIR}/ooxml"

if [[ ! -f "${MANIFEST}" ]]; then
  echo "error: ooxml_parser_rs manifest not found at ${MANIFEST}" >&2
  exit 1
fi

mkdir -p "${BIN_DIR}"
cargo build --release --manifest-path "${MANIFEST}"
SRC="$("${SCRIPT_DIR}/rust-release-bin.sh" "${MANIFEST}" ooxml)"
