...
class SkillRouter:
    ...
    def _pick_owner(self, by_layer: dict[str, list[ScoredSkill]]) -> ScoredSkill:
        ...
        top_owner_score = max((candidate.score for candidate in owner_candidates), default=float("-inf"))
    ...
    def _pick_overlay(self, task: str, allow_overlay: bool, selected_skill: SkillMetadata) -> SkillMetadata | None:
        ...
        for skill in self.skills:
            ...
            is_explicitly_mentioned = (
                _contains_phrase(task_tokens, skill.name)
                or _contains_phrase(task_tokens, skill.name.replace("-", " "))
                or any(
    ...
    def _score_skill(self, skill: SkillMetadata, task: str, first_turn: bool) -> ScoredSkill:
        ...
        gate_phrases = _gate_phrases(skill)
        matched_gates = [gate for gate in gate_phrases if _contains_phrase(task_token_list, gate)]
        if matched_gates:
