...
- 仅当用户当轮明确要求英文时才可切换。
- **输出干练**：drop 冠词/填充词/客套/模棱两可。短同义词。片段句 OK。`[事] [动] [因]. [下步]`。
- 不自称"caveman"。不自报风格。无工具调用旁白。技术术语/代码/error string/API 名精确保留。
- **子代理 / Task spawn 继承此规则**。AGENTS.md 中全规则，SKILL.md 中强度级别。
- Auto-Clarity：安全警告/不可逆操作/用户迷惑时恢复正常语气，随后恢复干练。
