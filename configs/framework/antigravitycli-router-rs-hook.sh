#!/usr/bin/env bash
# Canonical Antigravity CLI router-rs hook launcher (hooks.json merge must reference this path).
set -euo pipefail

HOOK_EVENT="${1:?usage: $0 <event>}"
HOOK_PAYLOAD="$(cat)"
ROOT="${ANTIGRAVITY_CLI_PROJECT_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
FW="${SKILL_FRAMEWORK_ROOT:-$ROOT}"

critical_event() {
  case "$(printf '%s' "$HOOK_EVENT" | tr '[:upper:]' '[:lower:]')" in
    sessionstart|pretooluse|userpromptsubmit|posttooluse|stop|subagentstart|subagentstop)
      return 0
      ;;
    *)
      return 1
...
emit_fail_closed_json() {
  local msg='router-rs binary unavailable for critical Antigravity CLI hook; fail-closed instead of silently bypassing gate enforcement'
...
if [ ! -x "${ROUTER_RS_BIN:-}" ]; then
  ...
  printf '%s\n' "[antigravity-cli-hook] router-rs binary unavailable for telemetry event $HOOK_EVENT; fail-open" >&2
  exit 0
fi

printf '%s' "$HOOK_PAYLOAD" | "$ROUTER_RS_BIN" host antigravity-cli hook lifecycle-context --repo-root "$ROOT"
