...
use observer_rs::{
    default_observer_config_path, default_observer_output_dir, default_telemetry_journal_path,
    load_config, run_analyze, run_health_score,
};
...
use observer_rs::ObserverConfig;

#[derive(Parser)]
#[command(name = "observer-rs")]
#[command(about = "High performance skill observer core", long_about = None)]
struct Cli {
...
enum Commands {
    ...
    /// Analyze telemetry journal and write artifacts/observer/analysis.json
    Analyze {
        #[arg(short, long, default_value = default_telemetry_journal_path())]
        journal: PathBuf,
        #[arg(short, long, default_value = default_observer_output_dir())]
        output_dir: PathBuf,
        #[arg(short, long, default_value_t = 30)]
        days: i64,
    },
...
fn resolve_window(days: i64, cfg: &ObserverConfig) -> i64 {
    if days == 30 {
        cfg.observer.audit_window_days
    } else {
        days
    }
}

fn resolve_config(cli: &Cli) -> anyhow::Result<ObserverConfig> {
    let path = cli
        .config
        .clone()
        .or_else(|| std::env::var("OBSERVER_RS_CONFIG").ok().map(PathBuf::from))
        .or_else(|| {
            let default = PathBuf::from(default_observer_config_path());
            if default.is_file() {
                Some(default)
            } else {
                None
...
mod tests {
    ...
    fn missing_journal_loads_as_empty_entries() {
        let missing = std::env::temp_dir().join(format!(
            "observer-rs-missing-journal-{}-{}.jsonl",
            std::process::id(),
    ...
    fn entry_is_recent_rejects_malformed_ts() {
        let entry = observer_rs::AuditJournalEntry {
            ts: "not-a-date".into(),
            ..Default::default()
        };
