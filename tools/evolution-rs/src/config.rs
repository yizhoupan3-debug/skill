...
pub struct ThresholdSection {
    ...
    #[serde(default = "default_min_entries_for_heal")]
    pub min_entries_for_heal: usize,
...
impl Default for ThresholdSection {
    fn default() -> Self {
        Self {
            ...
            min_entries_for_heal: default_min_entries_for_heal(),
...
fn default_min_entries_for_heal() -> usize {
    11
}
...
pub fn load_config(path: Option<&Path>) -> anyhow::Result<EvolutionConfig> {
    let Some(path) = path else {
        return Ok(EvolutionConfig::default());
    };
    if !path.is_file() {
        return Ok(EvolutionConfig::default());
    }
    let raw = fs::read_to_string(path)
        .with_context(|| format!("read evolution config {}", path.display()))?;
    let cfg: EvolutionConfig = toml::from_str(&raw)
        .with_context(|| format!("parse evolution config {}", path.display()))?;
    Ok(cfg)
}
...
mod tests {
    ...
    fn load_config_from_toml_overrides_defaults() {
        ...
        let path = dir.join("evolution.toml");
...
"#
        ...
        assert_eq!(cfg.evolution.audit_window_days, 14);
        assert_eq!(cfg.evolution.min_candidate_frequency, 3);
