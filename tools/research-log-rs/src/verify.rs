...
pub fn validate_research_state(path: &Path) -> Result<Vec<ValidationIssue>> {
    ...
    if let Some(status) = value.get("status").and_then(Value::as_str) {
        if !["active", "concluded", "archived"].contains(&status) {
            issues.push(ValidationIssue {
                ...
                message: format!("Unknown status '{}'; expected active|concluded|archived", status),
            });
        }
    }
    ...
    if let Some(stage) = value.get("stage").and_then(Value::as_str) {
        let known_stages = [
            "bootstrap", "inner_loop", "inner_loop_design",
            "inner_loop_code", "inner_loop_eval", "barrier_escalation",
            "finalize", "reflect",
        ];
...
"#).unwrap();
        ...
        assert!(issues.is_empty(), "valid state should have no issues: {:?}", issues);
    }
